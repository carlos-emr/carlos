...
2. **Database Check**: Run `mariadb -h db -uroot -ppassword carlos -e "SELECT user_name FROM security WHERE user_name='carlosdoc' LIMIT 1;"`
   - Expected: Shows `carlosdoc` user exists

3. **Pre-Test Cleanup** (REQUIRED for re-runs):
   ```bash
   mariadb -h db -uroot -ppassword carlos -e "DELETE FROM demographic WHERE last_name = 'TEST-UITEST2';"
   ```
...
3. Verify in database if record was created despite timeout:
   ```bash
   mariadb -h db -uroot -ppassword carlos -e "SELECT demographic_no, last_name FROM demographic WHERE last_name='TEST-UITEST2' ORDER BY demographic_no DESC LIMIT 1;"
   ```
