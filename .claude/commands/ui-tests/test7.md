...
2. **Database Check**: Run `mariadb -h db -uroot -ppassword carlos -e "SELECT hin FROM demographic WHERE demographic_no = 1;"`
   - Expected: Shows patient HIN

3. **Billing Codes Check**: Run `mariadb -h db -uroot -ppassword carlos -e "SELECT COUNT(*) FROM billingservice WHERE region = 'ON';"`
