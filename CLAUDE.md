...
make install --run-unit-tests       # Unit tests only (fast)
make install --run-tests            # All tests (modern + legacy)
mvn test -Dgroups="integration"     # By tag
mvn test -Dtest=MyDaoIntegrationTest # Specific class
```

**Documentation**: `docs/test/modern-test-framework-complete.md` | `docs/test/test-writing-guide.md` | `docs/test/test-writing-best-practices.md`
...
src/test-modern/java/io/github/carlos_emr/carlos/managers/   # Manager unit tests
src/test-modern/java/io/github/carlos_emr/carlos/test/       # Base classes & mocks
src/test-modern/resources/                                    # Test configs, persistence.xml, schema.sql

# Legacy Tests (JUnit 4) - reference only
src/test/java/io/github/carlos_emr/carlos/                   # Legacy test structure
```

**Use `/generate-tests`** for the complete test generation workflow. It covers:
- Infrastructure expansion (persistence.xml, test-context-full.xml, lookup tables)
- Hibernate/JPA duality (AbstractDaoImpl vs HibernateDaoSupport)
- Static mocking order for Manager tests
- BDD naming, tagging, and common pitfalls

The `inject-test-context.py` hook auto-injects test context when editing test files.
