...
public class Startup implements ServletContextListener {
	...
	// Filesystem path properties that must resolve to an existing, usable directory before
	// startup completes. PR #2710 stopped auto-creating runtime directories, so these are
	// validated up front to give operators a deterministic failure with clear remediation
	// instead of a partially broken instance that only fails later on first use.
	private static final String[] REQUIRED_DIRECTORY_PROPERTIES = {"BASE_DOCUMENT_DIR"};
    ...
    public void contextInitialized(ServletContextEvent sc) {
        ...
        try {
			...
			// Fail fast if a required filesystem path is missing or unusable, before any
			// dependent feature (document storage, etc.) tries to use it at runtime.
			for (String propName : REQUIRED_DIRECTORY_PROPERTIES) {
				validateRequiredDirectory(propName, p.getProperty(propName));
			}
    ...
    // Validates that a required path property points at an existing, readable and writable
    // directory. Aborts startup (via IllegalStateException, re-wrapped by the outer catch) with a
    // message naming the property, its configured value, the problem, and the fix. Configured paths
    // are filesystem locations, not secrets, so it is safe to include the value in the message.
    static void validateRequiredDirectory(String propName, String configuredPath) {
        if (configuredPath == null || configuredPath.trim().isEmpty()) {
            throw new IllegalStateException("Required path property " + propName + " is not set; refusing to start. Set "
                    + propName + " in carlos.properties to an existing, writable directory.");
        }
        File directory = new File(configuredPath.trim());
        if (!directory.exists()) {
            throw new IllegalStateException("Required directory for " + propName + " does not exist: " + configuredPath
                    + "; refusing to start. Create it and grant the CARLOS/Tomcat user access.");
        }
        if (!directory.isDirectory()) {
            throw new IllegalStateException("Required path for " + propName + " is not a directory: " + configuredPath
                    + "; refusing to start. Point " + propName + " at a directory, not a file.");
        }
        if (!directory.canRead() || !directory.canWrite()) {
            throw new IllegalStateException("Required directory for " + propName + " is not readable/writable: "
                    + configuredPath + "; refusing to start. Grant the CARLOS/Tomcat user read and write permission.");
        }
        logger.info("Validated required directory {} = {}", propName, configuredPath);
    }
