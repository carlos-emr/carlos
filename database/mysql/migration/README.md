...
`V1.0.3` onward is a forward delta. The highest version currently in use is `V1.0.27`
(`common/V1.0.27`, shared by both provinces), and the next free number for ANY
location — shared or province — is `V1.0.28`. The version line is global:
...
run — `common/V1.0.27` today on both provinces. A hypothetical new `bc/V1.0.11` would
apply fine on a fresh install (version order places it before `common/V1.0.27`) but would silently
