...
migration/
  ...
  common/  V1__baseline_schema.sql          # province-neutral tables (structure)
           ...
           V1.0.31__patient_portal_invite_delivery.sql
...
(`common/V1.0.31`, shared by both provinces). `V1.0.29` is taken by `release/2026.08`
(`common/V1.0.29__rename_placeholder_demo_clinic.sql`) and arrives here with that line's
forward-merge, so the next free number for ANY location — shared or province — is `V1.0.32`.
