...
migration/
  ...
  common/  V1__baseline_schema.sql          # province-neutral tables (structure)
           ...
           V1.0.30__patient_portal_security_objects.sql
           V1.0.32__portal_email_delivery.sql
...
(`common/V1.0.32`, shared by both provinces). `V1.0.31` is held by the portal invite delivery
migration (#3856), which must merge before `V1.0.32`. `V1.0.29` is taken by `release/2026.08`
(`common/V1.0.29__rename_placeholder_demo_clinic.sql`) and arrives here with that line's
forward-merge, so the next free number for ANY location — shared or province — is `V1.0.33`.
...
run — `common/V1.0.32` on both provinces once this line is merged. A hypothetical new `bc/V1.0.11`
would apply fine on a fresh install (version order places it before `common/V1.0.32`) but would silently
