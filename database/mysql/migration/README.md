...
migration/
  ...
  common/  V1__baseline_schema.sql          # province-neutral tables (structure)
           ...
           V1.0.24__rename_placeholder_demo_clinic.sql
...
`V1.0.3` onward is a forward delta. The highest version currently in use is `V1.0.24`
(`common/V1.0.24`, shared by both provinces), and the next free number for ANY
location — shared or province — is `V1.0.25`. The version line is global:
...
run — `common/V1.0.24` today on both provinces. A hypothetical new `bc/V1.0.11` would
apply fine on a fresh install (version order places it before `common/V1.0.24`) but would silently
