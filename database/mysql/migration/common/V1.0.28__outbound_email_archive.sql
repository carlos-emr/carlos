...
CREATE TABLE IF NOT EXISTS `outboundEmailArchive` (
    ...
    -- from an older OSCAR/OpenO schema that the engine is unknown, and V1.0.27 converts
...
-- no role at all. Fresh installs and legacy databases without a customized grant
-- therefore cannot use this gate. The second INSERT supplies the missing default;
-- an existing clinic grant (including an intentional denial) is preserved.
--
