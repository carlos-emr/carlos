...
- `V1.0.23__activate_legacy_consultation_services.sql` — repairs the untouched V1.0.2 Ontario service-catalogue seed, which shipped entirely inactive
...
The version line is global across `common` + `on`, so the next free number is `V1.0.32` (the
highest version in use, also on the Ontario path, is `common/V1.0.31` — see
