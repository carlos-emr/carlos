...
def cmd_db_apply_settings(argv) -> int:
    ...
    if tz and os.path.isfile(f"/usr/share/zoneinfo/{tz}"):
        ...
        if not _tz_resolves(tz):
            # NEVER write a drop-in the server cannot start with.
            warn(f"MariaDB still cannot resolve '{tz}' after loading the timezone tables; "
                 "NOT writing the default-time-zone drop-in")
            tz = ""
        verify_db_now = env_get(BACKUP_ENV, "CARLOS_BACKUP_VERIFY_DB") or "carlos_restore_drill"
        extra = ""
        if verify_db_now != "carlos_restore_drill":
            # binlog_ignore_db is ADDITIVE across option files: the packaged
            # 60- keeps the default drill schema out of the binlog, and this
            # generated line adds a renamed one. Without it, a custom
            # CARLOS_BACKUP_VERIFY_DB made every db-apply-settings restart
            # MariaDB for a value no restart could ever apply.
            extra = f"binlog_ignore_db = {verify_db_now}\n"
        ...
        try:
            ...
            pass
        if have != want_dropin:
            with open(tz_dropin, "w", encoding="utf-8") as fh:
                fh.write(want_dropin)
            log(f"rendered {tz_dropin} (default-time-zone = {tz})")
