...
def cmd_db_apply_settings(argv) -> int:
    ...
    tz_dropin = "/etc/mysql/mariadb.conf.d/61-carlos-emr-tz.cnf"
    if tz and os.path.isfile(f"/usr/share/zoneinfo/{tz}"):
        n = db_root(["-N", "-B", "-e", "SELECT COUNT(*) FROM mysql.time_zone_name"],
                    capture_output=True)
        if n.returncode == 0 and n.stdout.strip() == "0":
            log("loading the timezone tables into MariaDB (one-time, a few seconds)")
        ...
        want_dropin = (f"# Generated by carlos-ctl db-apply-settings — kept in step with\n"
                       f"# CARLOS_TZ in /etc/carlos-emr/carlos-emr.env. Do not edit here.\n"
