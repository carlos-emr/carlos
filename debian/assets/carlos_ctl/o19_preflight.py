...
PROFILE_BC = {
    ...
    'KNOWN_TABLES': {
        ...
        'bed_demographic_status': 'archive',
        'bed_type': 'copy',
        'bill_recipients': 'copy',
        'billactivity': 'copy',
        'billcenter': 'merge',
        'billing': 'copy',
        'billing_cdm_service_codes': 'copy',
        'billing_history': 'copy',
        'billing_msp_servicecode_times': 'copy',
        'billing_on_3rdPartyAddress': 'copy',
        'billing_on_item_payment': 'copy',
        'billing_on_payment': 'copy',
        'billing_on_premium': 'archive',
        'billing_on_transaction': 'copy',
        'billing_payment_type': 'merge',
        'billing_preferences': 'copy',
        'billing_private_transactions': 'copy',
        'billing_service_code_conditions': 'copy',
        'billing_trayfees': 'copy',
        'billingdetail': 'copy',
        'billinginr': 'copy',
        'billinglocation': 'reference',
        'billingmaster': 'copy',
        'billingnote': 'copy',
        'billingperclimit': 'copy',
        'billingreferral': 'copy',
        'billingservice': 'merge',
        'billingstatus_types': 'copy',
        'billingtypes': 'copy',
        'billingvisit': 'merge',
        'caisi_form': 'copy',
        'caisi_form_data': 'copy',
        ...
        'cssStyles': 'copy',
        'ctl_billingservice': 'merge',
        'ctl_billingservice_age_rules': 'copy',
        'ctl_billingservice_premium': 'merge',
        'ctl_billingservice_sex_rules': 'copy',
        'ctl_billingtype': 'archive',
        'ctl_diagcode': 'merge',
        'ctl_doc_class': 'merge',
        'ctl_doctype': 'merge',
        'ctl_document': 'copy',
        'ctl_frequency': 'merge',
        'ctl_servicecodes_dxcodes': 'copy',
        'ctl_specialinstructions': 'merge',
        'custom_filter': 'copy',
        'custom_filter_assignees': 'copy',
        'custom_filter_providers': 'copy',
        ...
        'formBCAR2007': 'copy',
        'formBCAR2012': 'copy',
        'formBCBirthSumMo': 'copy',
        'formBCBirthSumMo2008': 'copy',
        'formBCClientChartChecklist': 'copy',
        'formBCHP': 'copy',
        'formBCINR': 'copy',
        'formBCNewBorn': 'copy',
        'formBCNewBorn2008': 'copy',
        'formBPMH': 'copy',
        'formCESD': 'copy',
        'formCaregiver': 'copy',
        'formConsult': 'copy',
        ...
        'formInternetAccess': 'copy',
        'formLabReq': 'copy',
        'formLabReq07': 'archive',
        'formLabReq10': 'archive',
        'formLateLifeFDIDisability': 'copy',
        'formLateLifeFDIFunction': 'copy',
        'formMMSE': 'copy',
        'formMentalHealth': 'copy',
        ...
        'teleplanS23': 'copy',
        'teleplanS25': 'copy',
        'teleplan_adj_codes': 'copy',
        'teleplan_refusal_code': 'copy',
        'teleplan_response_log': 'copy',
        'teleplan_submission_link': 'copy',
        'tickler': 'copy',
        'tickler_category': 'merge',
        'tickler_comments': 'copy',
        'tickler_link': 'copy',
...
def run_checks(query, properties=None, province="on", accepted=(),
    ...
    # --- province gate ----------------------------------------------------
    # Each generated profile above was curated for ONE province and
    # stamps which (O19_PROFILE). A host assessed against another
    # province's rulings would get the wrong table classes, the wrong
    # patient-data list and the wrong dropped columns -- quietly, since
    # every ruling would still be a ruling.
    #
    # bind() points the per-province names at this host's profile and is
    # a no-op for a province this build does not carry, so the assertion
    # below is what refuses that host. Binding here rather than in main()
    # keeps the two together: the function that gates is the function
    # that binds, and a caller reaching run_checks by any other route
    # (the tests do) cannot get an unbound assessment.
    bind(province)
    if province != O19_PROFILE:
        findings.append(finding(
            ...
            "--accept flag exists for this."))
    elif province not in SUPPORTED_PROVINCES:
        # carried is not supported: the rulings for this province exist
        # and are reviewable, but no end-to-end rehearsal has run against
        # them yet, so the assessment must not read as "you may migrate".
        findings.append(finding(
            "province", BLOCKER,
            "the {0!r} manifest profile has not completed an end-to-end "
            "migration rehearsal".format(province),
            "Every table ruling for this province is curated and shipped, "
            "but the import verb refuses a province until a full "
            "rehearsal has taken a clinic database of that province from "
            "P0 to a passing verification. Supported in this build: "
            "{0}. No --accept flag exists for this."
            .format(", ".join(SUPPORTED_PROVINCES))))
