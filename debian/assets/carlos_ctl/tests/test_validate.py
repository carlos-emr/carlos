...
class TestFrontDoorListeners(unittest.TestCase):
    ...
    def listeners(self, *lines, bind_ip="127.0.0.1"):
        def output(command):
            if command == ["ss", "-ltnHp"]:
                return "\n".join(lines)
            self.fail("front-door probe ran an unexpected command: " + repr(command))
        with patch.object(validate, "out", side_effect=output):
            return [validate._nginx_listeners(port) for port in ("80", "443")]
    ...
    def test_a_neighbouring_port_is_not_the_front_door(self):
        # endswith(":443") also matched ":8443" once, and reported the front
        # door up when only some other service was.
