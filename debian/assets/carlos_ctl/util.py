...
# SKIP_EFORM_RENDERER development build, so readers still treat it as optional.
CHROMIUM_DIR = os.path.join(LIB, "chromium")
# Render browser service settings, 0640 root:carlos-render. Generated by the
# carlos-emr postinst when the render browser is present.
RENDER_BROWSER_ENV = os.path.join(CONF_DIR, "renderer.env")
# How an operator reinstalls carlos-emr. The packages ship as release files, not
# from an apt repository, so a bare `apt install --reinstall carlos-emr` finds
# nothing to download and changes nothing.
REINSTALL_HINT = "sudo apt install --reinstall ./carlos-emr_<version>_amd64.deb"


def render_payload_installed(chromium_dir: str = CHROMIUM_DIR) -> bool:
    """Whether the eForm render browser is usable: both binaries executable.

    The single definition init-config and `carlos-ctl check` share, and the
    same test the postinst applies (`[ -x ]`) before it generates
    renderer.env, so the three cannot disagree about whether a browser
    is installed.
    """
...
def reset_emr_start_limit() -> None:
    ...
    loop, but a single apt transaction restarts it more than once (carlos-emr plus
    the drugref postinst), and applying config changes means
