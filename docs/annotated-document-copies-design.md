...
| Persistence | Annotations are **not** stored as data. Saving composes a new PDF and files it as a **new `document` row** for the same patient. The original file is never modified, and neither is a filed copy: after a save the marks stay on the viewer page and can still be moved, edited or deleted, but saving again files another new copy rather than changing the one just filed. | Keeps the received document immutable as a clinical record. Removes the need for an annotation table, DAO, and Flyway migration. Matches the maintainer decision that a saved annotation *is* a new document. |
...
- Tools: **select** (move any mark; click to delete it, or to edit a text/date note), **text**, **highlight** (drag a rectangle; when the page has word boxes, the rectangle snaps to the words it covers), **draw** (pointer events → simplified polyline), **signature** (places the stored stamp; the drawing modal is kept for providers with no stamp and posts to the existing `ProviderSignatureStamp2Action`), **date** (today, editable). Text, date and signature marks can also be dragged from the placing tools (text, date, signature), and a click on a text or date note re-opens it for editing from select, text or date (clearing it removes the note), so marks can be adjusted without leaving the tool in use. A click that does not move a signature in a placing tool still places the new mark there. The drawing tools (highlight, draw) never grab, because strokes are routinely drawn across existing marks; ink and highlights move only in select. Moves are clamped so the mark stays on its page, matching the parser's `x + w <= 1` bound; a note is sized to its full text width, spaces included, as the composer measures it (note text keeps its whitespace), so a short note reaches the right margin and a note that would overrun the page (placed, moved or edited) is pulled back onto it, since the composer refuses text past the page edge. Notes are measured in the annotation font; a note placed or edited before that font has loaded is refitted once it arrives.
- Page navigation and page count from `numberofpages`.
- **Save** and **Save and fax**. After save the page shows the new document number and a link to it. Nothing on the page can modify a saved document: the marks stay on the page and can still be moved, edited or deleted, and any such change re-enables Save, which files another new copy. While a save is in flight, marks cannot be added, moved, edited or deleted, and Save is disabled while a mark is being dragged or a stroke is being drawn, so a save never snapshots the page mid-gesture. A drag is tracked in page coordinates and its preview is kept across redraws, so a scroll, a resize or the annotation font arriving mid-drag leaves the mark under the pointer. Every gesture (a stroke, a move, a click on a mark) acts as the tool and colour it started with, so a second pointer switching the toolbar mid-gesture changes nothing about it.
