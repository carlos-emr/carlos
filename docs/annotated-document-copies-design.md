...
3. **Bounded time.** Composition runs on a dedicated executor with a hard 60 second deadline and is cancelled on expiry, logging the document number only. **Page rendering has no timeout yet**: the megapixel ceiling above bounds the output size, which is the practical lever, but a pathological parse could still occupy a request thread. Adding the same executor treatment to `showPage` touches a legacy path shared by the whole document module and is deliberately left as a follow-up rather than folded into this change.
