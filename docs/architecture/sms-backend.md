...
Only `_msgSMS` is enforced today, by `CarlosSmsMessageBodyAuthorizationService`. `_sms` and `_admin.sms` are seeded ahead of the code (`V1.0.31__add_sms_security_objects.sql`) so the grants exist before the first gate ships; nothing checks them yet. `_msgSMS` is seeded by `V1.0.25`. Grants take effect on the next request: privileges are read from `secObjPrivilege` on every check and nothing caches them, so no restart is needed after the migration. Both migrations leave an existing clinic row untouched, including a bare `o` (no-rights) row; at equal priority `o` withholds the right for that role only, and a provider who also holds a role with a grant still passes. A patient-scoped `_sms$<demographicNo>` row with `o` additionally locks the session, as for every object. A clinic that wants a role to view history without sending grants it `r` on `_sms`. `_admin` = `x` confers nothing on `_admin.sms`; dotted objects need their own row.

When the send and history actions land, a failed check throws the paren form, `throw new SecurityException("missing required sec object (_sms)");`, in Struts actions and in REST or SOAP services alike (`docs/soap-rbac-hardening.md`); `CarlosExceptionMappingInterceptor` recognises refusals by that message. The one exception is the existing `_msgSMS` gate, which throws `AccessDeniedException` from `CarlosSmsMessageBodyAuthorizationService`. A send or history action must also require `_demographic` `r` for the patient, passing the `demographicNo` rather than `null`: `hasPrivilege` consults the patient-specific `_demographic$<id>` override only on the scoped call. Many legacy actions still pass `null`; do not copy them.
