...
- `database/mysql/migration/common/V1.0.17__performance_indexes_2.sql`
- `database/mysql/migration/bc/V1.0.18__bc_billingmaster_indexes.sql`
...
### `common/V1.0.17__performance_indexes_2.sql`

New indexes (each justified by an actual DAO predicate):

| Index | Table (columns) | Justification |
|---|---|---|
| `idx_demographicArchive_demographic_no` | `demographicArchive` (`demographic_no`) | `DemographicArchiveDaoImpl` lines 59, 75, 114, 128 — all four queries filter `demographicNo`; runs on every demographic edit/history view. Table was PK-only, so each call scanned the whole ever-growing archive. |
...
### `bc/V1.0.18__bc_billingmaster_indexes.sql`
...
| `idx_billingmaster_demographic_no` (`demographic_no`) | `BillingmasterDAO` 308 (`demographicNo + billingCode + billingstatus NOT IN`), 333 (`demographicNo ORDER BY billingmaster_no DESC`) |
...
2. Flyway CLI 11.14.0 (SHA-256 pinned as in CI) `migrate` + `validate` with
   ...
   Ontario — 18 migrations applied, schema at v1.0.17, validated; British Columbia — 16 migrations
   applied, schema at v1.0.18, validated.
...
4. Idempotency: V1.0.17 re-applied to both databases and V1.0.18 re-applied to BC through the
