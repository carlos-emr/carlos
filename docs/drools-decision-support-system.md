...
|-----------|--------------------------|--------|-------------------|
| **Flowsheet rules** | `MEASUREMENT_DS_DIRECTORY` filesystem | -- | Classpath `/oscar/oscarEncounter/oscarMeasurements/flowsheets/` |
...
**Location**: `src/main/resources/oscar/oscarEncounter/oscarMeasurements/flowsheets/*.drl`
...
**Location**: `src/main/resources/oscar/oscarEncounter/oscarMeasurements/flowsheets/decisionSupport/*.drl`
...
**Location**: `src/main/resources/oscar/oscarEncounter/oscarMeasurements/flowsheets/decisionSupport/test*.drl`
...
Located in `src/main/resources/oscar/oscarEncounter/oscarMeasurements/flowsheets/`
...
Located in `src/main/resources/oscar/oscarEncounter/oscarMeasurements/flowsheets/decisionSupport/`
...
1. Create or edit a DRL file in `src/main/resources/oscar/oscarEncounter/oscarMeasurements/flowsheets/`
...
1. Create a DRL file in `src/main/resources/oscar/oscarEncounter/oscarMeasurements/flowsheets/decisionSupport/`
...
All CARLOS DRL files use Drools 7.x DRL syntax:
...
| `org.drools.io.RuleBaseLoader.loadFromInputStream()` | `DroolsHelper.loadFromInputStream()` | Custom wrapper using `KieHelper` |
| `org.drools.io.RuleBaseLoader.loadFromUrl()` | `DroolsHelper.loadFromUrl()` | Custom wrapper using `KieHelper` |
...
| Class | Purpose |
|-------|---------|
| `DroolsHelper` | Centralizes `KieHelper`-based DRL compilation with error checking |
| `DroolsCompilationException` | Checked exception for compilation failures (replaces silent `null` returns) |

### Key Design Decisions

1. **`KieHelper` over `KieServices`**: Uses `KieHelper` for compilation, which creates self-contained compilations without installing modules in the global KIE repository. This prevents unbounded metadata growth in long-running server processes.
...
### MVEL Compatibility Fix (Java 21)

The Drools 7.74.1 runtime depends on MVEL (expression language) for internal compilation. The transitive dependency `mvel2:2.4.15.Final` references `java.lang.Compiler`, a class removed in JDK 16 (JEP 410). This causes a `NoClassDefFoundError` at runtime on Java 17+.

**Fix**: Override `mvel2` to `2.5.2.Final` in `pom.xml`:
```xml
<properties>
...
The `mvel2:2.5.x` line removes the `java.lang.Compiler` reference and is the upstream-recommended fix for running Drools 7.x on modern JDKs.
