...
The callback uses the transport's `EmailSendingException.isDeliveryOutcomeUncertain()` contract. A definite rejection revokes the unsent password; an uncertain outcome or unexpected runtime exception leaves it pending. The decision is never inferred from exception message text.
