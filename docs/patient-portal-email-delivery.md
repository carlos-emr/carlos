...
A value other than `true` or `false` blocks encrypted email (it never falls back to the manual password), while unencrypted email keeps working and the composer tells staff the setting is invalid. The same happens when `patient_portal.email.enabled=true` but the Portal connection itself cannot be used: switched off, a mistyped `patient_portal.enabled`, or only some of the `patient_portal.*` connection settings present. CARLOS checks that the full connection settings build, and the composer shows the misconfiguration alert up front instead of every encrypted send failing later with "password could not be prepared". Encryption itself refuses a blank password on every path.
...
What recovery may do is decided once, by `PortalEmailDeliveryService.classify`, from the portal state **and** the email status, and the page offers only that. A PENDING email younger than 15 minutes may still be sending, so recovery waits. A SENDING email whose status is SUCCESS was accepted after a lost state write, so recovery only publishes; one whose status is FAILED definitely was not sent, so it only revokes. Only a SENDING email with no recorded outcome asks staff to check the provider and confirm it. A confirmed acceptance is recorded as RESOLVED, like any other operator-asserted outcome, rather than SUCCESS; as with manual resolution, no chart note is written for it. A published password on a FAILED email, or a revoked one on a delivered email, can only arise when staff confirm an outcome while a slow send is still running; it is shown as inconsistent and keeps its outbox badge. The waiting window counts from when the email was created, not from when sending began. Manual **Resolve** is not offered for these emails; portal recovery owns them. A state that contradicts the status offers no automatic action. Every send and recovery outcome is audited without the password.
