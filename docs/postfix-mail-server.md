...
Terminal control characters are shown as escapes when these commands write to a
terminal; redirecting their output to a file preserves the original bytes.
...
Rerun the command with the flag after every allowlist change. `mail status`
shows this active generated map, so it does not mistake unapplied source edits
...
- **Capture directory:** `/var/log/carlos-mail-capture` (`root:carlos-mail`, `0750`)
- **Temporary spool:** `/var/log/carlos-mail-capture/.spool` (`carlos-mail:carlos-mail`, `0770`)
