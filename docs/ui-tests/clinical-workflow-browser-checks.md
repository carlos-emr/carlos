...
|---|---|---|
| `appointment-lifecycle` | Appointment HTML label preview/print, edit, advance status from the day sheet, cancel, delete an appointment (+ the `appointmentArchive` row) | `schedule-quick-search-appointment` and `echart-new-patient-notes` cover **booking**; nothing covered what happens to a booking afterwards |
...
printed label contents. It enters through the appointment Label link, checks
...
is preserved for clinics with calibrated label stock.
