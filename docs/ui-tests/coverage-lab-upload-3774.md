...
`addFile` recorded the checksum before the lab was parsed and saved, and `ABCDParser.save` writes the report, patient, routing and result rows through separate DAOs. A failure part-way used to leave partial rows and the checksum committed, so a retry was answered `uploadedPreviously` although the lab was never fully stored. The CML action no longer claims through `addFile`. It checks `isFileRecorded`, parses the file (which writes nothing), then records the checksum with the new, non-swallowing `FileUploadCheck.recordFile` and saves the lab in one `TransactionTemplate`, which the `REQUIRED` DAO writes join. The checksum therefore exists exactly when the lab does: a failure rolls both back, so a retry stores the lab; a commit whose outcome is unknown left both or neither; and there is no separate cleanup step that could itself fail.

The request holds `addFile`'s monitor (`FileUploadCheck.class`) from the duplicate check until the lab and checksum have committed or rolled back. Every lab uploader claims through `addFile`, so no other upload in the same application instance can see or claim the content in between. The lock covers one application instance, not several servers. The inside-lab and PathNet uploaders still keep a checksum when their own processing fails; that older behaviour is outside this change.

Unit tests cover each branch: `LabUpload2ActionUnitTest` (9 tests, run under Spring's real commit and rollback lifecycle) and `FileUploadCheckUnitTest` (5 tests).
