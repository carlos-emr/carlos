...
Unit tests cover each branch: `FileUploadCheckUnitTest` (13 tests, including `storeIfNew` stored, duplicate, rejected, thrown, lookup-failure and cannot-start cases, an upload of other content completing while one is being stored, and `addFile` for the same bytes waiting for it), `LabUpload2ActionUnitTest` for CML (9), `InsideLabUpload2ActionUnitTest` (6) and PathNet `LabUpload2ActionUnitTest` (6), all run under Spring's real commit and rollback lifecycle through the shared `RecordingTransactionManager` test helper. All 817 unit tests in the `lab` packages pass.
