...
`storeIfNew` holds `addFile`'s monitor (`FileUploadCheck.class`) from the duplicate check until the upload and checksum have committed or rolled back. Every lab uploader claims through `addFile` or `storeIfNew`, so no other upload in the same application instance can see or claim the content in between. The lock covers one application instance, not several servers.
