...
| `lab-requisition-links` | `lab/ViewLinkReq`, `messenger/SendDemoMessage` | Search ▸ Master Record ▸ E-Chart ▸ Urinalysis ▸ Msg/Req#; dated 2007/2010 requisitions and an unlinked report; save/reopen/change/unlink and patient context |
...
| `billing-on-group-disk-zero-total` **(landed, issue #3942)** | Administration ▸ Billing ▸ Generate OHIP diskette (`ViewBillingONMRI`) ▸ All Providers ▸ Create Report ▸ the new disk's OHIP-file link | A group member whose claims total $0 is on the group disk (its `HEB` batch, `HEH`/`HET` records; claims flip to `B` and link to the disk's `billing_on_header`); a member with no claims leaves no batch header; `billing_on_filename` summaries match | Throwaway billing group (three providers cloned from 999998) and two seeded claims in an isolated service-date window; all rows removed | `ViewBillingONMRI`, `ViewOngenreport`, `OscarDownload` |
