...
| UI-04 | Medium | Accessibility defects cause the patient to select, delete, or trust the wrong record. | Screen-reader names/state, focus management, scalable text, non-color trust indicators, confirmations for destructive actions, and undo where safe. | Automated WCAG checks plus manual VoiceOver, TalkBack, Windows, and macOS accessibility evaluation. | Automated WCAG A/AA gate covers primary browser states at desktop/phone widths and detected contrast failures are fixed; manual assistive-technology review remains |
