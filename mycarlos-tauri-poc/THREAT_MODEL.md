...
| UI-03 | High | An unlocked session remains visible on a shared or unattended device. | Configurable short inactivity lock; immediate manual lock; background/suspend lock; reauthentication for sensitive export/recovery actions. | Clock, sleep, process-resume, multi-window, and fast-user-switching tests. | Manual, 15-minute, and background locking implemented; target-device lifecycle matrix open |
