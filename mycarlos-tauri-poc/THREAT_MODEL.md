...
| STO-03 | High | Crash or power loss produces a partially committed record or destroys the previous valid state. | Transactional metadata; redundant identical manifest states; repair before orphan cleanup; write-verify-rename/commit sequence. | Kill the process at every persistence boundary and restart repeatedly. | Redundant manifest commits, corrupt-latest recovery, and failed-batch tests implemented; crash injection open |
