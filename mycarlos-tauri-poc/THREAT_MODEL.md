...
| STO-03 | High | Crash, low disk, or power loss produces a partially committed record or destroys the previous valid state. | Transactional metadata; redundant generation-bound headers and identical manifest states; unambiguous committed/degraded outcomes; recovery-mode read access; safe orphan cleanup; write-verify-rename/commit sequence. | Terminate the process and inject write failures at persistence boundaries, then restart repeatedly. | Subprocess termination and deterministic `NoSpace` matrices cover chunk/staging/rename/manifest/deletion boundaries; physical power-cut and full-filesystem matrix remain |
