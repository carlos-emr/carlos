...
test('label PDF url follows a new_label_print wrapper iframe but stays in the application', () => {
  const wrapper = 'http://127.0.0.1:8080/carlos/demographic/ViewPrintDemoLabel?demographic_no=7';
  assert.equal(resolvePdfUrl(wrapper, null), wrapper);
  assert.equal(resolvePdfUrl(wrapper, 'printDemoLabelAction?demographic_no=7&appointment_no='),
    'http://127.0.0.1:8080/carlos/demographic/printDemoLabelAction?demographic_no=7&appointment_no=');
  assert.equal(resolvePdfUrl(wrapper, '/carlos/report/GenerateEnvelopes?demos=7'),
    'http://127.0.0.1:8080/carlos/report/GenerateEnvelopes?demos=7');
  assert.throws(() => resolvePdfUrl(wrapper, 'https://example.com/label.pdf'), /another origin/);
});
