...
(async () => {
  ...
  try {
    ...
    assert(!/HTTP Status 500|Exception Report|CARLOS has encountered an unexpected error/i.test(popupBodyText), `Empty attachment submit crashed the popup: ${popupBodyText}`);
    assert(/ok/i.test(popupBodyText), `Empty attachment submit did not complete cleanly: ${popupBodyText}`);
    ...
    console.log(`PASS rtl empty attachment submit is stable and popup requestId matches saved fdid (${fdid})`);
  } catch (error) {
