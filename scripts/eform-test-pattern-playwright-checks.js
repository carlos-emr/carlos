...
async function openSavedEformFromPatientList(context, recorder, fdid) {
  ...
  const candidateLinks = page.locator('#efmTable a[onclick*="fdid="], #efmTable a[href*="fdid="]');
  await candidateLinks.first().waitFor({ state: 'visible', timeout: 15000 });
  // nosemgrep: javascript.playwright.security.audit.playwright-evaluate-arg-injection.playwright-evaluate-arg-injection -- expectedFdid is generated by this local regression test and compared as a string
  const linkIndex = await page.locator('#efmTable a').evaluateAll((links, expectedFdid) => {
    const hasExactFdid = (value) => {
      let searchFrom = 0;
      while (searchFrom < value.length) {
        const index = value.indexOf('fdid=', searchFrom);
        if (index === -1) {
          return false;
        }

        const valueStart = index + 'fdid='.length;
        const valueEnd = valueStart + expectedFdid.length;
        const before = value[index - 1];
        const after = value[valueEnd];
        const hasParamStart = before === undefined || before === '?' || before === '&';
        const hasParamEnd = after === undefined || after === '&' || after === '\'' || after === '"';
        if (hasParamStart && hasParamEnd && value.slice(valueStart, valueEnd) === expectedFdid) {
          return true;
        }
  ...
  assert(linkIndex >= 0, `Could not find exact patient list link for fdid ${fdid}`);
  const link = page.locator('#efmTable a').nth(linkIndex);
  const [popup] = await Promise.all([
    context.waitForEvent('page', { timeout: 30000 }),
    link.click(),
  ]);
...
(async () => {
  ...
  try {
    ...
    const cleanupErrors = [];
    if (context) {
      await listSavedFdidsFromPatientList(context, recorder, expected.subject)
