...
async function workflow(s) {
  ...
  const link = admin.locator('a[onclick*="/admin/ViewLotNrSearchRecordsHtm"]');
  await revealAuditLink(admin, link, 20000);
  const search = await s.popup(admin, link, 'lot-search');
  await s.step('search from the administration menu returns the exact owned lot', async () => {
    ...
    await clickAndAwaitReload(search, search.locator('input[type="submit"]'));
  ...
  await s.step('a second search from results can submit and show no matching lot', async () => {
    ...
    await clickAndAwaitReload(search, search.locator('input[type="submit"]'));
    assert(await search.locator('form[name="searchlotnr"]').isVisible(), 'Search results form disappeared');
