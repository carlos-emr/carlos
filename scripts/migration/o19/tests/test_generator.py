...
class TestGenerator(unittest.TestCase):
    ...
    def test_generated_manifests_carry_no_wall_clock_stamp(self):
        for name in self.gen.MANIFEST_FILES.values():
            text = (self.gen.MANIFEST_DIR / name).read_text(encoding="utf-8")
            self.assertNotIn("GENERATED_AT", text)
            self.assertNotIn("generated_at", text)

    def test_shipped_manifests_carry_the_format_and_kind(self):
        # the loader in carlos-ctl refuses anything else before reading a
        # data key; the envelope is the whole cross-package contract
        import json
        for kind, name in self.gen.MANIFEST_FILES.items():
            data = json.loads((self.gen.MANIFEST_DIR / name).read_text(
                encoding="utf-8"))
            self.assertEqual(data["format"], self.gen.MANIFEST_FORMAT, name)
            self.assertEqual(data["kind"], kind, name)

    def test_shipped_manifests_are_what_the_emitter_writes(self):
        # emit_json is the only writer: byte-stable re-serialisation of
        # the checked-in content (the --check contract, minus the O19
        # checkout the tables themselves need)
        import json
        for kind, name in self.gen.MANIFEST_FILES.items():
            text = (self.gen.MANIFEST_DIR / name).read_text(encoding="utf-8")
            ...
            self.assertEqual(self.gen.emit_json(kind, body), text, name)

    def test_the_preflight_block_renders_from_the_shipped_manifest(self):
        # the checked-in JSON must render to a block the standalone file
        # can carry (79 columns, every name present)
        import json
        text = (self.gen.MANIFEST_DIR / "o19_preflight.json").read_text(
            encoding="utf-8")
        data = json.loads(text)
        block = self.gen.render_preflight_block(data)
        for name in self.gen.PREFLIGHT_BLOCK_NAMES:
            self.assertIn("\n{0} = ".format(name), block)
        self.assertTrue(block.startswith(self.gen.MARKER_BEGIN))
        self.assertTrue(block.endswith(self.gen.MARKER_END))

    def test_the_cli_renders_the_block_byte_for_byte_like_the_generator(self):
        # carlos_ctl.o19manifest.render_preflight_block is a copy of the
        # generator's; --write-standalone depends on the two agreeing
        import json
        from carlos_ctl import o19manifest
        data = json.loads((self.gen.MANIFEST_DIR / "o19_preflight.json")
                          .read_text(encoding="utf-8"))
        self.assertEqual(o19manifest.render_preflight_block(data),
                         self.gen.render_preflight_block(data))
        self.assertEqual(tuple(o19manifest.PREFLIGHT_BLOCK_NAMES),
                         tuple(self.gen.PREFLIGHT_BLOCK_NAMES))
        self.assertEqual(tuple(o19manifest.PREFLIGHT_PROFILE_NAMES),
                         tuple(self.gen.PREFLIGHT_PROFILE_NAMES))
...
class TestOnePackageCarriesEveryProvince(unittest.TestCase):
    ...
    def test_with_no_second_province_profiles_is_empty(self):
        # bind() lives outside the block (hand-written in o19_preflight.py)
        # and still returns the default, so the callers need no
        # province-count special case
        block = self.emit({"on": {"tables": self.tables(),
                                  "stock_role_names": ["doctor"]}})
        self.assertIn("PROFILES = {}", block)
        self.assertNotIn("def bind(province):", block)
    ...
    def test_the_snapshot_names_exactly_what_bind_rebinds(self):
        ...
        emitted = list(o19_preflight._PROFILE_NAMES)
    ...
    def test_the_shipped_schema_manifest_snapshot_matches_too(self):
        # the same contract on the schema loader: the names it snapshots
        # for bind() are the names the generator repeats per province
        self.assertEqual(list(o19map_schema._PROFILE_NAMES),
                         list(self.gen.PROFILE_NAMES))
        for province, profile in o19map_schema.PROFILES.items():
            self.assertEqual(sorted(profile), sorted(self.gen.PROFILE_NAMES),
                             province)
