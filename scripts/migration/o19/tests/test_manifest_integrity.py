...
class TestPropsManifest(unittest.TestCase):
    ...
    def test_every_key_the_deployment_writes_is_ruled(self):
        ...
        config = (Path(o19map_schema.__file__).parent
...
class TestOverlayRulingsReachTheManifest(unittest.TestCase):
    ...
    def stale(bucket):
        return ("overrides_schema.py {0} disagrees with the shipped "
                ...
                "commit debian/assets/o19-manifest/ with the overlay change."
                .format(bucket))
    ...
    def test_verbatim_overlay_constants_are_copied_through(self):
        ...
        for name in ("CREDENTIAL_TABLES",
            ...
            # JSON-shaped on both sides: the overlay writes tuples, the
            # shipped manifest carries them as lists
            self.assertEqual(json.loads(json.dumps(getattr(o19map_schema, name))),
                             json.loads(json.dumps(getattr(self.ov, name))),
                             self.stale(name))
