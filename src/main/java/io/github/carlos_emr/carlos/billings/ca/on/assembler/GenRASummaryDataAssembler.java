 ...
 *
 * <p>Like {@link GenRADescDataAssembler}, this is mutation-on-render —
...
public final class GenRASummaryDataAssembler {
    ...
    public GenRASummaryViewModel assemble(HttpServletRequest request, LoggedInInfo loggedInInfo) {
        ...
        GenRASummaryViewModel.Builder b = GenRASummaryViewModel.builder().raNo(nullToEmpty(raNoStr));
        if (raNo == null) {
            return b.build();
        }
        ...
        List<GenRASummaryViewModel.ReportRow> rows = new ArrayList<>();
    ...
    private List<GenRASummaryViewModel.ProviderOption> loadProviderOptions(Integer raNo, String selected) {
        List<GenRASummaryViewModel.ProviderOption> options = new ArrayList<>();
        for (Object[] res : raDetailDao.search_raprovider(raNo)) {
            ...
            options.add(new GenRASummaryViewModel.ProviderOption(ohipNo, displayName, isSelected));
        }
        return options;
    }
    ...
    private GenRASummaryViewModel.ReportRow buildRow(RaDetail rad,
        ...
        GenRASummaryViewModel.Category category;
        if ("02".equals(location)) {
            category = GenRASummaryViewModel.Category.HOSPITAL;
            ...
            category = GenRASummaryViewModel.Category.LOCAL_CLINIC;
            totals.clinic = totals.clinic.add(paid);
        } else {
            category = GenRASummaryViewModel.Category.OTHER;
            totals.other = totals.other.add(paid);
        }

        return new GenRASummaryViewModel.ReportRow(
