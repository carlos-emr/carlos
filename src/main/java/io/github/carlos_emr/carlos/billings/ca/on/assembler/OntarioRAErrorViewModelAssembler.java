...
/**
 * Assembles {@link OntarioRAErrorViewModel} for
...
public class OntarioRAErrorViewModelAssembler {
    ...
    public OntarioRAErrorViewModelAssembler(BillingRAReportService prep) {
        this.prep = prep;
    }
    ...
    public OntarioRAErrorViewModel assemble(String raNoParam, String proNoParam) {
        OntarioRAErrorViewModel.Builder b = OntarioRAErrorViewModel.builder();
        ...
        List<OntarioRAErrorViewModel.ProviderOption> providers = new ArrayList<>(
                rawProviders == null ? 0 : rawProviders.size());
        if (rawProviders != null) {
            for (Object o : rawProviders) {
                Properties prop = (Properties) o;
                providers.add(new OntarioRAErrorViewModel.ProviderOption(
        ...
        if (!selectedProvider.isEmpty() && !"all".equals(selectedProvider)) {
            ...
            List<OntarioRAErrorViewModel.ErrorRow> rows = new ArrayList<>(
                    errorList == null ? 0 : errorList.size());
            if (errorList != null) {
                for (Properties prop : errorList) {
                    rows.add(new OntarioRAErrorViewModel.ErrorRow(
