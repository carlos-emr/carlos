...
/**
 * Assembles {@link OntarioRAViewModel} for {@code billing/CA/ON/onGenRA.jsp},
...
public class OntarioRAViewModelAssembler {

    private final SecurityInfoManager securityInfoManager;
    private final BillingONRemittanceAdviceService dbObj;

    public OntarioRAViewModelAssembler(SecurityInfoManager securityInfoManager,
     ...
     */
    public OntarioRAViewModel assemble(HttpServletRequest request, LoggedInInfo loggedInInfo) {
        ...
        List<OntarioRAViewModel.Row> rows = new ArrayList<>(raList == null ? 0 : raList.size());
        if (raList != null) {
            for (Properties p : raList) {
                rows.add(new OntarioRAViewModel.Row(
        ...
        return OntarioRAViewModel.builder().rows(rows).build();
    }
}
