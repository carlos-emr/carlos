...
public final class ChartClinicalSummaryProvider implements ClinicalSummaryArtifactProvider {
    ...
    public ClinicalSummaryArtifact load(LoggedInInfo user, ClinicalSummaryRequest request) {
        ...
        warning(artifact, "chart_scope", "Chart evidence includes available clinical modules and all eligible note history. Read-access restrictions, unreadable material and unsupported formats are reported below. Missing information is not a negative clinical finding. This extract is not model inference.");

        ArrayNode medicationClaims = section(artifact, "medications", "Recorded prescription history");
        if (security.hasPrivilege(user, "_rx", "r", demographicNo)) {
            for (Drug drug : SpringUtils.getBean(RxManager.class).getDrugs(user, demographicNo, RxStatus.ALL)) {
                ...
                String text = "Medication: " + value(drug.getDrugName())
                        ...
                        + "\nArchived record: " + drug.isArchived();
                addRecord(artifact, patientId, "rx-" + drug.getId(), "Prescription record",
                        date(drug.getRxDate()), text, text, "Medication", medicationClaims);
            }
            warning(artifact, "prescription_status", "Prescriptions, including archived history, are recorded orders, not confirmation of current use. Review dates, instructions and status flags against the chart.");
        } else {
        ...
        ArrayNode allergyClaims = section(artifact, "allergies", "Recorded allergy history");
        if (security.hasPrivilege(user, "_allergy", "r", demographicNo)) {
            for (Allergy allergy : SpringUtils.getBean(AllergyDao.class).findAllergies(demographicNo)) {
                ...
                String text = "Allergy: " + value(allergy.getDescription())
                        ...
                        + "\nArchived record: " + allergy.getArchived();
        ...
        ArrayNode noteClaims = section(artifact, "notes", "Signed encounter note history");
        ...
        if (program == null || !program.toString().matches("[1-9][0-9]{0,8}")) {
            ...
            List<CaseManagementNote> candidates = notes.getNotes(String.valueOf(demographicNo));
            for (CaseManagementNote note : candidates) {
                requirePatient(String.valueOf(demographicNo), note.getDemographic_no());
            }
            List<CaseManagementNote> eligible = candidates.stream()
            ...
            for (CaseManagementNote note : notes.filterNotes(user, user.getLoggedInProviderNo(), eligible, program.toString())) {
                ...
                addRecord(artifact, patientId, "note-" + note.getId(), "Signed encounter note",
                        date(note.getObservation_date()), body, body, "Encounter note", noteClaims);
            }
            warning(artifact, "note_scope", "Notes are filtered using CARLOS program, role and configured facility access. Unsigned, archived, locked, blank or unclassified-role notes are omitted. All eligible latest note revisions are included without a count or text-length cutoff.");
        }
        new SupplementalClinicalSummarySources(security).append(user, demographicNo, artifact);
        return new ClinicalSummaryArtifact(artifact);
    }

    static ArrayNode section(ObjectNode artifact, String id, String title) {
        return ((ArrayNode) artifact.get("sections")).addObject().put("id", id).put("title", title).putArray("claim_ids");
    }

    static void addRecord(ObjectNode artifact, String patientId, String id, String title,
    ...
    static void addSource(ObjectNode artifact, String patientId, String id, String title,
        ...
        ((ArrayNode) artifact.get("coverage")).addObject().put("source_id", id)
                ...
                .put("reason", cited ? "Recorded source content reproduced without model inference."
                        : "Patient identity only; no clinical claim.");
    }

    static void warning(ObjectNode artifact, String code, String message) {
        ((ArrayNode) artifact.get("validation")).addObject().put("severity", "warning")
                .put("code", code).put("message", message).putArray("source_ids");
    }

    static void requirePatient(Object expected, Object actual) {
