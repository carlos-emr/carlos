...
public interface ProviderSiteDao extends AbstractDao<ProviderSite> {
    ...
    /**
     * Finds providers with status {@code "1"} that share at least one site with
     * the given provider number.
     *
     * <p>The source provider is included when it is active and assigned to one
     * of its own sites. A null provider number, or one with no matching site
     * assignments, returns an empty list. Results are ordered by last name, then
     * first name.</p>
     *
