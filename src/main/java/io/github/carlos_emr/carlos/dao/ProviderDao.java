...
/**
 * Provider-specific DAO that extracts database access logic from provider-related
 * JSP files into a centralized, reusable data access layer.
 *
 * <p>Extends {@link OscarSuperDao} and supplies a registry of named SQL queries
 * covering appointment scheduling, provider search, demographic intake,
 * clinical list queries, multi-site schedule lookups, and patient roster
 * operations. Queries are executed through the parent class using
 * Spring's {@link org.springframework.jdbc.core.JdbcTemplate}.</p>
 *
 * <p>This class is intended to be accessed indirectly through
 * {@code OscarSuperManager} rather than called directly from JSP or
 * action classes.</p>
 *
...
public class ProviderDao extends OscarSuperDao {
    ...
    /** Row mappers keyed by query name, used for object-based result mapping. */
    ...
    /**
     * Named SQL query registry. Each entry is a two-element array: {@code [queryName, sqlStatement]}.
     * Queries are grouped by functional area: appointment scheduling, provider/group lookup,
     * demographic and encounter search, schedule management, multi-site operations,
     * patient intake data, roster status, and clinical list (cl_) summary queries.
     */
    private String[][] dbQueries = new String[][]{
            ...
            // -- Appointment and preference update queries --
            ...
            // -- Demographic and encounter lookup queries --
            ...
            // -- Provider search and team queries --
            ...
            // -- Schedule date and holiday queries --
            ...
            // -- Appointment timecode and template queries --
            ...
            // -- Group schedule availability count --
            ...
            // -- Multi-site queries for schedule day view page --
            ...
            // -- Patient intake queries (pharmacy, allergies, medications, demographics, measurements) --
            ...
            // Retrieves the latest unarchived reminder note for a patient by joining case management tables;
            // issue_id 38 corresponds to the "Reminders" issue type
            ...
            // -- Roster status and clinical list (cl_) summary queries --
            {"search_rsstatus", "select distinct roster_status from demographic where roster_status not in ('', 'RO', 'NR', 'TE', 'FS')"},
            // Computes patient age inline using MySQL date arithmetic on year/month/day of birth columns
            ...
            // Same as cl_demographic_query but additionally filtered by roster status
            ...
            // Counts appointments within the last year for the given patient
    ...
    /**
     * Returns the provider-specific SQL query registry for use by
     * {@link OscarSuperDao} query execution methods.
     *
    ...
    /**
     * Returns the provider-specific row mapper registry for use by
     * {@link OscarSuperDao#executeRowMappedSelectQuery(String, Object[])}.
     *
