...
public class EFormBrowserPdfService {
    ...
    private Path renderWithSlot(int fdid, String providerId) throws PDFGenerationException {
        ...
        try {
            ...
            driver = createDriver(buildChromeOptions(resolveChromiumPath(), unsandboxed, allowedOrigin));
    ...
    private ChromeDriver createDriver(ChromeOptions options) throws PDFGenerationException {
        ...
        try {
            ...
            if (sandboxEnabled()) {
                // The operator opted into Chromium's OS sandbox (EFORM_RENDER_SANDBOX=true) but it could
                // not start (kernel without unprivileged user namespaces, or Chromium refusing the sandbox
                // as root). Fail closed rather than silently degrading to --no-sandbox: tell the operator
                // how to make the sandbox real, or how to consciously drop back to the default posture.
                throw new PDFGenerationException(
                        ...
                        + "Chromium's sandbox can start, or unset EFORM_RENDER_SANDBOX to run with --no-sandbox "
                        + "where the container itself provides isolation.", e);
            }
