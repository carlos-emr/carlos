...
public final class EFormRenderPdfHtmlComposer {
    ...
    static final String LEGACY_MEASUREMENT_ELEMENT_ID = "carlos-legacy-measurement-history";
    ...
    static String buildPdfHtml(EForm eForm, List<EFormValue> eFormValues, String contextPath, String projectHome, EFormRenderTokenService.RenderToken renderToken) {
        ...
        html = embedLegacyMeasurementHistory(html, eForm);
     ...
     */
    static String embedLegacyMeasurementHistory(String html, EForm eForm) {
        ...
        return embedLegacyMeasurementSeries(html, series);
    }

    /**
     * The half of {@link #embedLegacyMeasurementHistory} below the database, split out so the markup
     * contract — which is what actually breaks — is testable without a data source.
     *
     * @param series one {@code who_measurements} value: {@code date|ht|wt|circ|comments;…}
     */
    static String embedLegacyMeasurementSeries(String html, String series) {
        if (html == null || !html.contains(LEGACY_MEASUREMENT_ROUTE)) {
            return html;
        }
        return insertBeforeBodyEnd(html, legacyMeasurementPayloadElement(series == null ? "" : series));
    }
     ...
     */
    static String legacyMeasurementMarkup(String series, int column) {
     ...
     */
    static String legacyMeasurementPayloadElement(String series) {
        ObjectNode payload = new ObjectMapper().createObjectNode();
        for (Map.Entry<String, Integer> column : LEGACY_MEASUREMENT_COLUMNS.entrySet()) {
            payload.put(column.getKey(), legacyMeasurementMarkup(series, column.getValue()));
        }
