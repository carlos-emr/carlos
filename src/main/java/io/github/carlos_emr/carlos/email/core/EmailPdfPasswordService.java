...
public class EmailPdfPasswordService {
    ...
    static final int GROUP_PAIR_COUNT = 2;
    static final int DEFAULT_WORD_COUNT = WORD_GROUP_COUNT * GROUP_PAIR_COUNT;
    static final int DEFAULT_DIGIT_COUNT = DIGIT_GROUP_COUNT * GROUP_PAIR_COUNT;
    static final int DIGIT_BOUND = 10;
    static final String SEPARATOR = "-";
    static final Pattern WORD_PATTERN = Pattern.compile("[a-z]+");
    private static final Pattern WORDLIST_ROW_PATTERN = Pattern.compile("(\\d{4})\\t([a-z]+)");
    ...
    public String generatePassphrase() {
        List<String> parts = new ArrayList<>(DEFAULT_WORD_COUNT + GROUP_PAIR_COUNT);
        for (int i = 0; i < GROUP_PAIR_COUNT; i++) {
    ...
    private static List<String> loadWordsFromResource(String resourcePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                Matcher matcher = WORDLIST_ROW_PATTERN.matcher(trimmedLine);
                if (!matcher.matches()) {
                    throw new IllegalStateException(
                            "Email PDF password wordlist contains an invalid row: " + trimmedLine);
                }
                String expectedIndex = String.format(Locale.ROOT, "%04d", loadedWords.size());
                if (!expectedIndex.equals(matcher.group(1))) {
                    throw new IllegalStateException(
                            "Email PDF password wordlist contains an unexpected row index: " + trimmedLine);
                }
                loadedWords.add(matcher.group(2));
            }
        } catch (IOException e) {
