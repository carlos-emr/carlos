        ...
        if (!securityInfoManager.hasPrivilege(loggedInInfo, "_lab", "r", null)) {
            ...
            for (int j = 0; j < labs.size(); j++) {
                ...
                if (result.isMDS()) {
                    ...
                    url = request.getContextPath() + "/oscarMDS/SearchPatient.do?demographicId=" + bean.demographicNo + "&providerNo=" + bean.providerNo + "&segmentID=" + result.segmentID + "&multiID=" + result.multiLabId + "&status=" + result.getReportStatus();
