...
public final class FrmSetupForm2Action extends ActionSupport {
    ...
    // FindSecBugs HRS_REQUEST_PARAMETER_TO_HTTP_HEADER: formName reaches the redirect Location header, but only after isAllowedSetupFormName() has matched it against VALID_FORM_NAME_PATTERN (^[a-zA-Z0-9_]+$) and returned 400 otherwise, so CR/LF can never reach the header.
    @SuppressFBWarnings(value = {"IMPROPER_UNICODE", "PATH_TRAVERSAL_IN", "UNVALIDATED_REDIRECT", "HRS_REQUEST_PARAMETER_TO_HTTP_HEADER"}, justification = "case-insensitive comparison of an internal/domain value (status/flag/enum/MIME/code); not a security or authorization decision; and path validated for directory containment via PathValidationUtils before use. UNVALIDATED_REDIRECT: redirect target is a same-origin application path or validated internal path, not an attacker-controlled external URL. HRS_REQUEST_PARAMETER_TO_HTTP_HEADER: formName is allowlisted against ^[a-zA-Z0-9_]+$ before the redirect, so it cannot carry CR/LF")
