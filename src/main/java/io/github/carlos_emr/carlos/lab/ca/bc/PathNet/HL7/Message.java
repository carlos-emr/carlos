...
public class Message {
     ...
     * when none can be resolved: that is a data condition, and the lab must still be stored. A
     * failure writing the routing row itself propagates, so the caller's transaction rolls the
     * whole message back instead of committing it unrouted.
     */
    ...
    ////
    /**
     * Links the message to its patient, or to demographic 0 (unmatched) when the PID cannot be
     * matched or parsed; as in {@link #linkToProvider}, only a failure to write the routing row
     * propagates.
     */
