...
public class RxUtil {
    ...
    private static final String[] DURATION_UNITS_QTY = {"[0-9]++\\s+(?i)days\\s", "[0-9]++\\s+(?i)weeks\\s", "[0-9]++\\s+(?i)months\\s", "[0-9]++\\s+(?i)day\\s", "[0-9]++\\s+(?i)week\\s", "[0-9]++\\s+(?i)month\\s", "[0-9]++\\s+(?i)d\\s", "[0-9]++\\s+(?i)w\\s", "[0-9]++\\s+(?i)m\\s", "[0-9]++\\s+(?i)mo\\s", "[0-9]++\\s+(?i)days$", "[0-9]++\\s+(?i)weeks$", "[0-9]++\\s+(?i)months$", "[0-9]++\\s+(?i)day$", "[0-9]++\\s+(?i)week$", "[0-9]++\\s+(?i)month$", "[0-9]++\\s+(?i)d$", "[0-9]++\\s+(?i)w$", "[0-9]++\\s+(?i)m$", "[0-9]++\\s+(?i)mo$",
    ...
    private static final Pattern[] DURATION_UNITS_QTY_PATTERNS = new Pattern[DURATION_UNITS_QTY.length];

    static {
        for (int i = 0; i < DURATION_UNITS.length; i++) {
            DURATION_UNITS_PATTERNS[i] = Pattern.compile(DURATION_UNITS[i]);
        }
        for (int i = 0; i < DURATION_UNITS_QTY.length; i++) {
            DURATION_UNITS_QTY_PATTERNS[i] = Pattern.compile(DURATION_UNITS_QTY[i]);
        }
    }
    ...
    public static String getDurationFromQuantityText(String qStr) {
        String retStr = "";
        for (Pattern p : DURATION_UNITS_QTY_PATTERNS) {
            Matcher m = p.matcher(qStr);
            if (m.find()) {
                String foundStr = (qStr.substring(m.start(), m.end())).trim();
