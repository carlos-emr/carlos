...
/**
 * Programmatic Drools rule numerator using {@code isDataEqualTo} string equality
 * comparison with date range filtering for CARLOS EMR clinical reporting.
 *
 * <p>DroolsNumerator5 is similar to {@link DroolsNumerator4} but uses
 * {@code "isDataEqualTo"} instead of {@code "getDataAsDouble"} for the
 * {@link TargetCondition} type. This means the measurement value is compared
 * as an <strong>exact string match</strong> rather than a numeric comparison,
 * which is appropriate for categorical or coded measurements (e.g. "positive",
 * "negative", "Y", "N").</p>
 *
 ...
 *   <li>Build a {@link TargetCondition} of type {@code "isDataEqualTo"} to
 *       perform a string equality comparison of the measurement against the
 *       expected value.</li>
 ...
 * <ul>
 *   <li>{@code "measurements"} -- the measurement type code (e.g. "FLU_VACCINE")</li>
 *   <li>{@code "value"} -- the expected string value for equality comparison (e.g. "Y")</li>
 ...
 * <ul>
 *   <li>{@link DroolsNumerator4} -- Same date range filtering but uses
 *       {@code getDataAsDouble} for numeric comparison instead of string equality</li>
 *   <li>{@link DroolsNumerator2} -- Uses {@code getDataAsDouble} without date range
 *       filtering</li>
 * </ul>
 *
...
public class DroolsNumerator5 implements Numerator {
    ...
    /**
     * Creates a new instance of DroolsNumerator5 with all fields defaulting to {@code null}.
     */
     ...
     * an {@code isDataEqualTo} string equality comparison with date range filtering.
     *
     ...
     *   <li>{@code "value"} -- the expected string value for equality comparison</li>
     *   <li>{@code "startDate"} -- start of the date range ({@code yyyy-MM-dd})</li>
     *   <li>{@code "endDate"} -- end of the date range ({@code yyyy-MM-dd})</li>
     * </ul>
     *
     * <p>The key difference from {@link DroolsNumerator4} is the use of
     * {@code "isDataEqualTo"} instead of {@code "getDataAsDouble"}, making this
     * variant suitable for categorical or coded measurement values.</p>
     *
     ...
     *         matches the expected value exactly (i.e. is "in range"), {@code false}
    ...
    public boolean evaluate(LoggedInInfo loggedInInfo, String demographicNo) {
        ...
        try {
            ...
            // Extract measurement type, expected value, and date range from replaceable values
            ...
            // Build a TargetCondition that uses "isDataEqualTo" for string equality
            // comparison (unlike DroolsNumerator4 which uses "getDataAsDouble" for numeric)
     ...
     * into this numerator before evaluation. For DroolsNumerator5, the expected keys
     ...
     * For DroolsNumerator5, this must contain:
     * <ul>
     *   <li>{@code "measurements"} -- the measurement type code</li>
     *   <li>{@code "value"} -- the expected string value for equality comparison</li>
     *   <li>{@code "startDate"} -- start of the date range ({@code yyyy-MM-dd})</li>
