...
public class SQLReporter implements Reporter {
    ...
    public boolean generateReport(HttpServletRequest request) {
        ...
        if (curReport instanceof ReportObjectGeneric) {
            ...
            MiscUtils.getLogger().error("Report template {} uses legacy non-parameterized SQL path — execution blocked for security", templateId);
            request.setAttribute("errormsg", "Error: This report template uses an unsupported legacy format. Please update the template to use parameterized queries.");
            request.setAttribute("templateid", templateId);
            return false;
        }
    ...
    public boolean generateSequencedReport(HttpServletRequest request) {
        ...
        if (curReport instanceof ReportObjectGeneric) {
            ...
            MiscUtils.getLogger().error("Report template {} uses legacy non-parameterized SQL path (sequenced) — execution blocked for security", templateId);
            request.setAttribute("errormsg", "Error: This report template uses an unsupported legacy format. Please update the template to use parameterized queries.");
            request.setAttribute("templateid", templateId);
            return false;
        }
     ...
     * @param sqlParams   JDBC parameters for the prepared statement
