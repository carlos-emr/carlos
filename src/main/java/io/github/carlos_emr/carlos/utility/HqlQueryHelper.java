...
public final class HqlQueryHelper {
     ...
     *
     * <p>Note: A named-parameter overload also exists that accepts a {@code Map} instead of
     * varargs. Java resolves overloads correctly since {@code Map} is more specific than
     * {@code Object}, but callers should use explicit types to avoid confusion.</p>
     *
     * @param session the Hibernate session from {@code currentSession()}
     * @param hql the HQL query string using 1-based positional parameters ({@code ?1}, {@code ?2}, etc.)
