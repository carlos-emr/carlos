 ...
 * eChart encounter and needs the toolbar to initialize; the JAX-RS
 * {@code AuthenticationInInterceptor} on this {@code /rs} surface already
 * requires a valid session for every method here. {@code saveSettings}
...
public class OceanService extends AbstractServiceImpl {
    ...
    @Autowired
    private OceanSettingDao oceanSettingDao;
    ...
    public OceanSettingsTo1 saveSettings(OceanSettingsTo1 request) {
        ...
        String settings = request == null ? null : request.getSettings();
        OceanSetting saved = oceanSettingDao.saveSettings(settings);
