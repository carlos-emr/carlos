...
public class WorkFlowDSFactory {
    ...
    public static WorkFlowDS getWorkFlowDS(String workflow) {
        ...
        kieBase = loadRuleBase(workflow);
        return new WorkFlowDS(kieBase);
    }
    ...
    public static KieBase loadRuleBase(String string) {
        ...
        try {
            ...
            if (!fileFound) {
                MiscUtils.getLogger().debug("Loading workflow from classpath resources");
                URL url = WorkFlowDSFactory.class.getResource("/oscar/oscarWorkflow/rules/" + string);
                MiscUtils.getLogger().debug("URL instantiated: {}", url);
