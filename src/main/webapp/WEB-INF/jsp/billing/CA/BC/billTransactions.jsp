<%@page import="java.util.*" %>
<%@ page import="io.github.carlos_emr.carlos.entities.BillHistory" %>
<%@ page import="io.github.carlos_emr.carlos.entities.Provider" %>
<%@ page import="io.github.carlos_emr.carlos.billings.ca.bc.data.BillingHistoryDAO" %>
<%@ page import="io.github.carlos_emr.carlos.billings.ca.bc.MSP.MSPReconcile" %>
<%@ taglib uri="/WEB-INF/security.tld" prefix="security" %>
<%@ taglib uri="jakarta.tags.fmt" prefix="fmt" %>
<fmt:setBundle basename="oscarResources"/>
<%
    String roleName$ = (String) session.getAttribute("userrole") + "," + (String) session.getAttribute("user");
    boolean authed = true;
%>
<security:oscarSec roleName="<%=roleName$%>" objectName="_report,_admin.reporting,_admin" rights="r"
                   reverse="<%=true%>">
    <%authed = false; %>
    <%response.sendRedirect(request.getContextPath() + "/securityError?type=_report&type=_admin.reporting&type=_admin");%>
</security:oscarSec>
<%
    if (!authed) {
        return;
    }
%>

<%
    java.text.NumberFormat nf = java.text.NumberFormat.getCurrencyInstance();
    String billNo = request.getParameter("billNo");
    String billMasterNo = request.getParameter("billMasterNo");
    MSPReconcile rec = new MSPReconcile();
    BillingHistoryDAO dao = new BillingHistoryDAO();
    List billingTransactions = new ArrayList();
    if (billNo != null) {
        billingTransactions = dao.getBillHistoryByBillNo(billNo);
    } else if (billMasterNo != null) {
        billingTransactions = dao.getBillHistory(billMasterNo);
    }
%>
<link href="<%= request.getContextPath() %>/share/css/oscar.css" rel="stylesheet"
      type="text/css"/>
<link rel="stylesheet" type="text/css" media="all" href="<%= request.getContextPath() %>/share/css/extractedFromPages.css"/>
<table width="100%">
    <tr class="SectionHead">
        <td colspan="8" class="bCellData"><fmt:message key="billing.billTransactions.tdBilltransaction"/></td>
    </tr>
    <tr class="ColHead">
        <td><fmt:message key="billing.billTransactions.tdStat"/></td>
        <td><fmt:message key="billing.billTransactions.tdSeq"/></td>
        <td><fmt:message key="billing.billTransactions.tdIns"/></td>
        <td><fmt:message key="billing.billTransactions.tdPract"/></td>
        <td><fmt:message key="billing.billTransactions.tdBillamt"/></td>
        <td><fmt:message key="billing.billTransactions.tdType"/></td>
        <td><fmt:message key="billing.billTransactions.tdAmtadj"/></td>
        <td><fmt:message key="billing.billTransactions.tdUpdated"/></td>
    </tr>
    <%
        for (Iterator iter = billingTransactions.iterator(); iter.hasNext(); ) {
            BillHistory item = (BillHistory) iter.next();
            Provider provider = rec.getProvider(item.getPractitioner_no(), 0);
    %>
    <tr align="center">
        <td><%=rec.getStatusDesc(item.getBillingStatus())%>
        </td>
        <td><%=item.getSeqNum()%>
        </td>
        <td><%=item.getBillingtype()%>
        </td>
        <td><%=provider.getInitials()%>
        </td>
        <%
            //for display purposes need to negate value so that
            //a postive internal adjustment appears as a "debit"
            if ("10".equals(item.getPaymentTypeId())) {
                item.setAmountReceived(item.getAmountReceived() * -1);
            }
        %>
        <td><%=nf.format(item.getAmount())%>
        </td>
        <td><%=item.getPaymentTypeDesc()%>
        </td>
        <td><%=nf.format(item.getAmountReceived())%>
        </td>
        <td><%=item.getArchiveDate()%>
        </td>
    </tr>
    <%}%>
</table>
