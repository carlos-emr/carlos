...
<%
    ...
    if (true) {
        out.clearBuffer();
        if (formId == 0) {
            request.getRequestDispatcher("/WEB-INF/jsp/form/formbcnewbornpg1.jsp?demographic_no=" + demoNo + "&formId=" + formId).include(request, response);
        } else {
            FrmRecord rec = (new FrmRecordFactory()).factory("BCNewBorn");
            java.util.Properties props = rec.getFormRecord(LoggedInInfo.getLoggedInInfoFromSession(request), demoNo, formId);

            request.getRequestDispatcher("/WEB-INF/jsp/form/formbcnewborn" + props.getProperty("c_lastVisited", "pg1")
