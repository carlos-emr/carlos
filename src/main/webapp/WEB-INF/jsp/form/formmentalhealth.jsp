...
<security:oscarSec roleName="<%=roleName$%>"
    ...
    <%
        ...
        if (true) {
            out.clearBuffer();
            if (formId == 0) {
                request.getRequestDispatcher("/WEB-INF/jsp/form/formmhreferral.jsp?demographic_no=" + demoNo + "&formId=" + formId).include(request, response);
            } else {
                FrmRecord rec = (new FrmRecordFactory()).factory("MentalHealth");
                java.util.Properties props = rec.getFormRecord(LoggedInInfo.getLoggedInInfoFromSession(request), demoNo, formId);

                request.getRequestDispatcher("/WEB-INF/jsp/form/formmh" + props.getProperty("c_lastVisited", "referral")
