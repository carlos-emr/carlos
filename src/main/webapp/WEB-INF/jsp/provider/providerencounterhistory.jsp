...
<center>
    ...
    <table width="90%" border="0">
        <tr>
            ...
            %> <a href=#
                  onClick="popupPage(600,800,'/provider/providercontrol.do?encounter_no=<%=enc.getId()%>&demographic_no=<%=Encode.forUriComponent(request.getParameter("demographic_no") != null ? request.getParameter("demographic_no") : "")%>&dboperation=search_encountersingle&displaymodevariable=<%=Encode.forUriComponent(strTemplateURL)%>&displaymode=vary&bNewForm=0')"><%=Encode.forHtml(historysubject)%>
            </a></font><br>
                <%
                } else if (strForm.compareTo("") != 0) {
                %> <a href=#
                      onClick="popupPage(400,600,'/provider/providercontrol.do?encounter_no=<%=enc.getId()%>&demographic_no=<%=Encode.forUriComponent(request.getParameter("demographic_no") != null ? request.getParameter("demographic_no") : "")%>&template=<%=Encode.forUriComponent(strForm)%>&dboperation=search_encountersingle&displaymode=encountersingle')"><%=Encode.forHtml(historysubject)%>
            </a></font><br>
                <%
                        }
                    }
