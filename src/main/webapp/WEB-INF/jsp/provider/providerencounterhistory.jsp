    ...
    <table width="90%" border="0">
        <tr>
            ...
            %> <a href=#
                ...
                %> <a href=#
                      onClick="popupPage(400,600,'<%= request.getContextPath() %>/provider/providercontrol?encounter_no=<%=enc.getId()%>&demographic_no=<e:forUriComponent value='<%= request.getParameter("demographic_no") != null ? request.getParameter("demographic_no") : "" %>' />&template=<carlos:encode value='<%= strForm %>' context="uriComponent"/>&dboperation=search_encountersingle&displaymode=encountersingle')"><carlos:encode value='<%= historysubject %>' context="html"/>
            </a></font><br>
                <%
                        }
                    }
