...
<script type="text/javascript" src="${ ctx }/library/jquery/jquery-ui-1.14.2.min.js" ></script>
        ...
        function showHideSpecInst(elementId){
			...
			function resetReRxDrugList() {
				...
				var url = ctx + "/rx/deleteRx.do?parameterValue=clearReRxDrugList";
				var data = "rand=" + rand;
           ...
           function popupRxSearchWindow(){
               ...
               var top = winY+70;
               var left = winX+110;
                var url = ctx + "/rx/searchDrug.do?rx2=true&searchString=" + encodeURIComponent(document.getElementById('searchString').value);
           ...
           function popupRxReasonWindow(demographic,id){
               ...
               var top = winY+70;
               var left = winX+110;
                var url = ctx + "/rx/RxReason.do?demographicNo=" + demographic + "&drugId=" + encodeURIComponent(id);
...
function renderRxStage() {
    ...
    function represcribeOnLoad(drugId){
        ...
        var url= ctx + "/rx/rePrescribe2.do";
    ...
    function moveDrugDown(drugId,swapDrugId,demographicNo) {
    	CarlosAjax.request('<c:out value="${ctx}"/>/rx/reorderDrug.do', {
  		  method: 'post',
  		  parameters: {method: 'update', direction: 'down', drugId: drugId, swapDrugId: swapDrugId, demographicNo: demographicNo},
  		  onSuccess: function(transport) {
  			callReplacementWebService("/rx/ViewListDrugs.do",'drugProfile');
            resetReRxDrugList();
            resetStash();
  		  }
  		});
	...
	function showPreviousPrints(scriptNo) {
                popupWindow(720, 700, ctx + '/rx/ViewShowPreviousPrints.do?scriptNo=' + scriptNo, 'ShowPreviousPrints')
	}
    ...
    function completeMedRec() {
   	 ...
   	 if(ok) {
					var url = ctx + "/rx/ViewCompleteMedRec.do";
    ...
    function printDrugProfile() {
    	...
    	if(ids.length>0) {
                    popupWindow(720, 700, ctx + '/rx/ViewPrintDrugProfile2.do?ids=' + ids.join(','), 'PrintDrugProfile');
    	} else {
                    popupWindow(720, 700, ctx + '/rx/ViewPrintDrugProfile2.do', 'PrintDrugProfile');
    	}
    }
                ...
                <td style="padding-right:15px;"><!--Column Two Row Two-->
                    ...
                    <div class="floatingWindow" id="reRxConfirmBox">
                        <p style="margin-bottom: 12px; font-size: 11px; text-align: end">
                            <fmt:message key="SearchDrug.msgReRxConfirmPrefix"/> <span style="font-weight: bold" id="selectedCount">0</span> <fmt:message key="SearchDrug.msgReRxConfirmSuffix"/>
                        </p>
                        <div style="display: flex; gap: 10px; justify-content: flex-end;">
                            <input type="button" name="cancel" class="ControlPushButton" value="<fmt:message key="SearchDrug.msgCancel"/>"
                                   onclick="cancelAndClearSelection()" title="<fmt:message key="SearchDrug.msgCancel"/>">
                            <input type="button" name="stage" class="ControlPushButton" value="<fmt:message key="SearchDrug.msgStageMedication"/>"
                                   onclick="stageSelectedReRxMedications()" title="<fmt:message key="SearchDrug.msgStageMedications"/>">
                        </div>
                    </div>
                    ...
                    <table style="border-collapse: collapse">
                        <tr id="medicationManagementRow">
							...
							<%if(securityManager.hasWriteAccess("_rx",roleName2$,true)) {%>
                                <form action="${pageContext.request.contextPath}/rx/searchDrug.do"  onsubmit="return checkEnterSendRx();" style="display: inline; margin-bottom:0;" id="drugForm" name="drugForm" method="post">
                                        ...
                                        <tr id="searchPrescriptionRow">
                                                    ...
                                                    <div id="advanceSearchParameters">
                                                        <fieldset id="drugCategorySet">
                                                            <input type="radio" id="allCategories" name="method"
                                                                   ...
                                                                   checked="checked"/>
                                                            <label for="allCategories"><fmt:message key="SearchDrug.drugCategory.all"/></label>

                                                            <input type="radio" id="brandName" name="method"
                                                                   value="searchBrandName" class="trigger" disabled/>
                                                            <label for="brandName"><fmt:message key="SearchDrug.drugCategory.brand"/></label>

                                                            <input type="radio" id="genericName" name="method"
                                                                   value="searchGenericName" class="trigger" disabled/>
                                                            <label for="genericName"><fmt:message key="SearchDrug.drugCategory.ingredient"/></label>
                                            ...
                                            <td>
                                                <div id="searchDrugsButtonSet">
                                                    <input type="button" name="search" class="btn btn-secondary btn-sm"  value="<fmt:message key="SearchDrug.msgSearch"/>" onclick="popupRxSearchWindow();" title="<fmt:message key="SearchDrug.help.Search"/>">
                                                    <input id="customDrug" type="button" class="btn btn-secondary btn-sm" onclick="customWarning2();" value="<fmt:message key="SearchDrug.msgCustomDrugRx3"/>" title="<fmt:message key="SearchDrug.help.CustomDrug"/>" />
                                                    <input id="customNote" type="button" class="btn btn-secondary btn-sm"  onclick="customNoteWarning();" value="<fmt:message key="SearchDrug.msgNoteRx3"/>" title="<fmt:message key="SearchDrug.help.CustomNote"/>"/>
                                                    <input id="reset" type="button" class="btn btn-secondary btn-sm" title="<fmt:message key="SearchDrug.help.clearPending"/>"   onclick="resetStash();" value="<fmt:message key="SearchDrug.msgResetPrescriptionRx3"/>"/>
                                                    <%if (CarlosProperties.getInstance().hasProperty("ONTARIO_MD_INCOMINGREQUESTOR")) {%>
                                                    <a href="javascript:goOMD();" title="<fmt:message key="SearchDrug.help.OMD"/>"><fmt:message key="SearchDrug.msgOMDLookup"/></a>
                                                    <%}%>
                                                    <security:oscarSec roleName="<%=roleName2$%>" objectName="_rx" rights="x">
                                                    <input id="saveButton" type="button"  class="btn btn-primary btn-sm" onclick="updateSaveAllDrugsPrintCheckContinue();" value="<fmt:message key="SearchDrug.msgSaveAndPrint"/>" title="<fmt:message key="SearchDrug.help.SaveAndPrint"/>" />
                                                    </security:oscarSec>
                                                    <input id="saveOnlyButton" type="button"  class="btn btn-secondary btn-sm" onclick="updateSaveAllDrugsCheckContinue();" value="<fmt:message key="SearchDrug.msgSaveOnly"/>" title="<fmt:message key="SearchDrug.help.Save"/>"/>
                                                </div>
                                            </td>
                                        ...
                                        <tr>
                                            <td>
                                                <div class="DivContentSectionHead">
                                                    ...
                                                    &nbsp;
													<%if(securityManager.hasWriteAccess("_rx",roleName2$,true)) {%>
                                                    <a href="#" onclick="var rp=document.getElementById('reprint');rp.style.display=(rp.style.display==='none')?'':'none';return false;"><fmt:message key="SearchDrug.Reprint"/></a>
                                                    &nbsp;
                                                    <a href="javascript:void(0);" id="cmdRePrescribe" onclick="RePrescribeLongTerm();" style="width: 200px" ><fmt:message key="SearchDrug.msgReprescribeLongTermMed"/></a>
                                                    &nbsp;
													<% } %>
                                                    <a href="javascript:popupWindow(720,920, ctx + '/rx/ViewChartDrugProfile.do?demographic_no=<%=demoNo%>','PrintDrugProfile2')"><fmt:message key="SearchDrug.timelineDrugProfile"/></a>
                                                    &nbsp;
                                                    &nbsp;&nbsp;
                                                </div>
                            ...
                            <%}}%>
                                                ...
                                                <table>
                                                    <tr>
                                                        <td>
                                                            <table class="legend">
                                                                    <tr>
                                                                        <td style="text-align: left; width:100px;">
                                                                            <a href="#"  title="<fmt:message key="provider.rxChangeProfileViewMessage"/>"
																	    ...
																	    <td>
																	       ...
																	       <table class="legend_items" align="left">
																			<tr>
																				<%if(show_current){%>
																				<td >
		                                                                            <a href="javascript:void(0);" onclick="callReplacementWebService('/rx/ViewListDrugs.do','drugProfile');CngClass(this);" 
																				...
																				<%}if(show_all){%>
	                                                                            <td >
																					<a href="javascript:void(0);" onclick="callReplacementWebService('/rx/ViewListDrugs.do?show=all','drugProfile');CngClass(this);" 
																				...
																				<%}if(active){%>
																				<td >
																					<a href="javascript:void(0);" onclick="callReplacementWebService('/rx/ViewListDrugs.do?status=active','drugProfile');CngClass(this);" 
																				...
																				<%}if(inactive){%>
																				<td >
																					<a href="javascript:void(0);" onclick="callReplacementWebService('/rx/ViewListDrugs.do?status=inactive','drugProfile');CngClass(this);" 
																				...
																				if(longterm_acute){%>
																				<td >
																					<a href="javascript:void(0);" onclick="callReplacementWebService('/rx/ViewListDrugs.do?longTermOnly=true&heading=Long Term Meds','drugProfile'); callAdditionWebService('/rx/ViewListDrugs.do?longTermOnly=acute&heading=Acute','drugProfile');CngClass(this);" 
																				...
																				<td >
																					<a href="javascript:void(0);" onclick="callReplacementWebService('/rx/ViewListDrugs.do?longTermOnly=true&heading=Long Term Meds','drugProfile'); callAdditionWebService('/rx/ViewListDrugs.do?longTermOnly=acute&heading=Acute&status=active','drugProfile');callAdditionWebService('/rx/ViewListDrugs.do?longTermOnly=acute&heading=Inactive&status=inactive','drugProfile');callAdditionWebService('/rx/ViewListDrugs.do?heading=External&drugLocation=external','drugProfile');CngClass(this);" 
                                                            ...
                                                            <div id="themeLegend">
                                                                <a href="javascript:void(0);" class="currentDrug"><fmt:message key="SearchDrug.legend.currentDrug"/></a> |
                                                                <%if(!CarlosProperties.getInstance().getProperty("rx.delete_drug.hide","false").equals("true")) {%>
                                                                <a href="javascript:void(0);" class="archivedDrug"><fmt:message key="SearchDrug.legend.archivedDrug"/></a> |
                                                                <%} %>
                                                                <a href="javascript:void(0);" class="expireInReference"><fmt:message key="SearchDrug.legend.expireInReference"/></a> |
                                                                <a href="javascript:void(0);" class="expiredDrug"><fmt:message key="SearchDrug.legend.expiredDrug"/></a> |
                                                                <a href="javascript:void(0);" class="longTermMed"><fmt:message key="SearchDrug.legend.longTermMed"/></a> |
                                                                <a href="javascript:void(0);" class="discontinued"><fmt:message key="SearchDrug.legend.discontinued"/></a> |
                                                        ...
                                                        <form action="${pageContext.request.contextPath}/rx/deleteRx.do" method="post">
                                    ...
                                    <table><!--drug profile, view and listdrugs.jsp-->
                                        <tr>
                                            <td>
                                                <div class="DivContentSectionHead">
                                                <fmt:message key="SearchDrug.otherMedications"/>
                                             </div>
                                            </td>
                                        </tr>
                                        <tr>
                                        	...
                                        	<table class="sortable" id="OMedsTabls" width="50%" border="0" cellpadding="3">
                                        		<tr>
                                        			<th align="left"><fmt:message key="SearchDrug.OMeds.dateEntered"/></th>
                                        			<th align="left"><fmt:message key="SearchDrug.OMeds.medication"/></th>
                                        		</tr>
                                        		 <%
                                        		// java.text.SimpleDateFormat formatter = new java.text.SimpleDateFormat("yyyy-MM-dd");
...
<div id="dragifm" style="top:0px;left:0px;"></div>
    <div id="discontinueUI" style="position: absolute;display:none;width:500px;height:200px;background-color:white;padding:20px;border:1px solid grey">
        <h3><fmt:message key="SearchDrug.discontinue.heading"/><span id="disDrug"></span></h3>
        ...
        <input type="button" onclick="document.getElementById('discontinueUI').style.display='none';" value="<fmt:message key="SearchDrug.discontinue.cancel"/>"/>
        <input type="button" onclick="Discontinue2(document.getElementById('disDrugId').value,document.getElementById('disReason').value,document.getElementById('disComment').value,document.getElementById('disDrug').textContent);" value="<fmt:message key="SearchDrug.discontinue.action"/>"/>
...
<script type="text/javascript">
        function changeLt(element, drugId) {
            ...
            const url = ctx + "/rx/WriteScript.do?parameterValue=updateLongTermStatus";
            CarlosAjax.request(url, {
                method: 'post',
                parameters: data,
                onSuccess: function (transport) {
   ...
   var modalBox=function(){
       this.show=function(randomId, displaySRC, H){
           if(!document.getElementById("xmaskframe")){
               ...
               //divFram.setAttribute("src","/rx/ViewDisplayMedHistory.do?randomId="+randomId);
    ...
    function updateProperty(elementId){
         ...
         if(randomId!=null){
             var url=ctx + "/rx/WriteScript.do?parameterValue=updateProperty";
             var el=document.getElementById(elementId);
             var data="";
    ...
    function setPrn(randomId){
     ...
     function updateSpecialInstruction(elementId){
         var randomId=elementId.split("_")[1];
         var url=ctx+ "/rx/WriteScript.do?parameterValue=updateSpecialInstruction";
    ...
    function resetStash(){
               var url=ctx + "/rx/deleteRx.do?parameterValue=clearStash";
    ...
    function rxPageSizeSelect(){
               ...
               var url=ctx + "/rx/GetRxPageSizeInfo.do?method=view";
    ...
    function reprint2(scriptNo){
        ...
        var url= ctx + "/rx/rePrescribe2.do?method=reprint2";
       CarlosAjax.request(url,
    ...
    function deletePrescribe(randomId){
        var data="randomId="+randomId;
        var url=ctx + "/rx/rxStashDelete.do";
   ...
   function Delete2(element){
             ...
             var url=ctx + "/rx/deleteRx.do?parameterValue=Delete2"  ;
   ...
   function checkAllergy(id,atcCode){
        const url = ctx + "/rx/showAllergy.do"
   ...
   function checkIfInactive(id,dinNumber){
    ...
    function Discontinue2(id,reason,comment,drugSpecial){
        var url=ctx + "/rx/deleteRx.do?parameterValue=Discontinue"  ;
    ...
    function updateCurrentInteractions(){
        CarlosAjax.request(ctx + "/rx/GetmyDrugrefInfo.do", {method:'post',parameters:"method=findInteractingDrugList&rand="+ Math.floor(Math.random()*10001),onSuccess:function(transport){
                            CarlosAjax.request(ctx + "/rx/ViewUpdateInteractingDrugs.do", {method:'post',parameters:"rand="+ Math.floor(Math.random()*10001),onSuccess:function(transport){
...
function popForm2(scriptId){
        try{
            var url = ctx + "/rx/viewScript.do?scriptId="+scriptId;
            var calcs = jQuery("#Calcs").val();
            if( calcs != null && calcs != "" ) {
                try {
                    var pharmacy = JSON.parse(calcs);
                    if( pharmacy != null && pharmacy.id != null ) {
                        url= ctx + "/rx/viewScript.do?scriptId="+scriptId+"&pharmacyId="+encodeURIComponent(pharmacy.id);
                    }
                } catch (e) {
                    oscarLog(e);
                }
...
function addFav(randomId,brandName){
   ...
   if (favoriteName.length > 0){
    ...
    function showHiddenRes(){
        ...
        if(resHidden2 == 0){
          ...
          var url = ctx + "/rx/ViewUpdateHiddenResources.do";
   ...
   // var totalHiddenResources=0;
    ...
    function ShowW(id,resourceId,updated){
				...
				var url = ctx + '/rx/GetmyDrugrefInfo.do';
...
function removeReRxDrugId(drugId) {
    if (drugId != null) {
        ...
        const url = ctx + "/rx/WriteScript.do";
        CarlosAjax.request(url, {method: 'post', parameters: data});
    }
}
...
function represcribe(element, toArchive){
    ...
    if(drugId!=null && document.getElementById("reRxCheckBox_"+drugId).checked === true){
        ...
        var urlUpdateId= ctx + "/rx/WriteScript.do";
...
function rePrescribe2(uiRefId, drugId) {
    ...
    const url = ctx + "/rx/rePrescribe2.do";
        CarlosAjax.updater('rxText', url, {
...
function addDrugToReRxListInSession(uiRefId, drugId) {
    ...
    const urlUpdateId = ctx + "/rx/WriteScript.do";
    CarlosAjax.request(urlUpdateId, {method: 'post', parameters: dataUpdateId});
}
...
function updateQty(element){
    ...
    function parseIntr(element){
        ...
        var url= ctx + "/rx/UpdateScript.do";
        var quantity="quantity_"+rand;
        var str;
       var methodStr="method_"+rand;
       var routeStr="route_"+rand;
...
<%--	%>--%>
    ...
    function updateSaveAllDrugsPrintContinue(){
        ...
        var url= ctx + "/rx/WriteScript.do?parameterValue=updateSaveAllDrugs&rand="+ Math.floor(Math.random()*10001);
        CarlosAjax.request(url,
