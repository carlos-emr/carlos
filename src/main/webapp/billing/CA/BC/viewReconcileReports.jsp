...
<security:oscarSec roleName="<%=roleName$%>" objectName="_report,_admin.reporting,_admin" rights="r"
                   reverse="<%=true%>">
    <%authed = false; %>
    <%response.sendRedirect(request.getContextPath() + "/error/SecurityError.do?type=_report&type=_admin.reporting&type=_admin");%>
</security:oscarSec>
<%
    if (!authed) {
        return;
   ...
   %>
     ...
     <tr>
        ...
        <td >&nbsp;&nbsp;
           ...
           <a href="genTAS00.jsp?rano=<%=Encode.forUriComponent(StringUtils.noNull(raNo))%>&proNo=" target="_blank">Detail</a> |
           <a href="genTAS22.jsp?rano=<%=Encode.forUriComponent(StringUtils.noNull(raNo))%>&proNo=" target="_blank">Summary</a>
