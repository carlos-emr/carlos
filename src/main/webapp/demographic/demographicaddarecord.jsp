...
<html>
    ...
    <div class="container">
        ...
        <form method="post" name="addappt">
            ...
            %>
            <script language="JavaScript">
                document.addappt.action = "<%= request.getContextPath() %>/appointment/addappointment.jsp?user_id=<%=Encode.forJavaScript(request.getParameter("creator") != null ? request.getParameter("creator") : "")%>&provider_no=<%=provider_no2%>&bFirstDisp=<%=bFirstDisp2%>&appointment_date=<%=request.getParameter("appointment_date")%>&year=<%=year2%>&month=<%=month2%>&day=<%=day2%>&start_time=<%=start_time2%>&end_time=<%=end_time2%>&duration=<%=duration2%>&name=<%=URLEncoder.encode(bufName.toString(), StandardCharsets.UTF_8)%>&chart_no=<%=URLEncoder.encode(bufChart.toString(), StandardCharsets.UTF_8)%>&bFirstDisp=false&demographic_no=<%=dem.toString()%>&messageID=<%=request.getParameter("messageId")%>&doctor_no=<%=Encode.forJavaScript(bufDoctorNo.toString())%>&notes=<%=Encode.forJavaScript(request.getParameter("notes") != null ? request.getParameter("notes") : "")%>&reason=<%=Encode.forJavaScript(request.getParameter("reason") != null ? request.getParameter("reason") : "")%>&location=<%=Encode.forJavaScript(request.getParameter("location") != null ? request.getParameter("location") : "")%>&resources=<%=request.getParameter("resources")%>&type=<%=request.getParameter("type")%>&style=<%=request.getParameter("style")%>&billing=<%=request.getParameter("billing")%>&status=<%=Encode.forJavaScript(request.getParameter("status") != null ? request.getParameter("status") : "")%>&createdatetime=<%=request.getParameter("createdatetime")%>&creator=<%=Encode.forJavaScript(request.getParameter("creator") != null ? request.getParameter("creator") : "")%>&remarks=<%=Encode.forJavaScript(request.getParameter("remarks") != null ? request.getParameter("remarks") : "")%>";
