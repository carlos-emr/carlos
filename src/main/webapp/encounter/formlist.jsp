...
<html>
    ...
    <center>
        <table BORDER="0" CELLPADDING="2" CELLSPACING="2" WIDTH="65%" BGCOLOR="white">
            ...
            %>
            <a
                    href="formlist.jsp?demographic_no=<%= Encode.forHtmlAttribute(demoNo) %>&limit1=<%=nLastPage%>&limit2=<%=strLimit2%>"><fmt:setBundle basename="oscarResources"/><fmt:message key="encounter.formlist.formLastpage"/></a>
            |
            <%
                }
                if (nItems == intLimit2) {
            %>
            <a
                    href="formlist.jsp?demographic_no=<%= Encode.forHtmlAttribute(demoNo) %>&limit1=<%=nNextPage%>&limit2=<%=strLimit2%>">
