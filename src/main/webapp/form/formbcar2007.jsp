...
<%
    ...
    if (true) {
        out.clearBuffer();
        if (formId == 0) {
            request.getRequestDispatcher("formbcar2007pg1.jsp?demographic_no=" + demoNo + "&formId=" + formId).include(request, response);
        } else {
            FrmRecord rec = (new FrmRecordFactory()).factory("BCAR");
            java.util.Properties props = rec.getFormRecord(LoggedInInfo.getLoggedInInfoFromSession(request), demoNo, formId);

            request.getRequestDispatcher("formbcar2007" + props.getProperty("c_lastVisited", "pg1")
