                ...
                <table id="summaryView" class="tablesorter">
                    ...
                    <tr id="labdoc_<%=segmentID%>" bgcolor="<%=bgcolor%>" <%if (result.isDocument()) {%>
                        ...
                        <td>
                            ...
                            %>
                            <a href="javascript:parent.reportWindow('<%=request.getContextPath()%>/lab/CA/ALL/createLabLabel.do?inWindow=true&segmentID=<%=segmentID%>&providerNo=<%=providerNo%>&searchProviderNo=<%=searchProviderNo%>&status=<%=status%>&showLatest=true')"><%=labRead%><%=Encode.forHtml(result.getPatientName())%>
                            </a>
                            <%
                                }
