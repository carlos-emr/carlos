...
<security:oscarSec roleName="<%=roleName$%>" objectName="_dxresearch" rights="r" reverse="<%=true%>">
    <%authed = false; %>
    <%response.sendRedirect(request.getContextPath() + "/error/SecurityError.do?type=_dxresearch");%>
</security:oscarSec>
<%
    if (!authed) {
        return;
...
<%@ page
        import="java.math.*, java.util.*, java.sql.*, io.github.carlos_emr.*, java.net.*, io.github.carlos_emr.carlos.dxresearch.bean.*" %>
<%@ page import="io.github.carlos_emr.carlos.dxresearch.bean.dxResearchBean" %>
