 ...
 *
 * Adds demographicNo to CARLOS/Prototype/jQuery AJAX calls and Rx-related
 * form submissions so that RxSessionFilter (server-side) can swap in the
 * correct per-patient RxSessionBean for each request.
 *
 * Requires: var currentDemographicNo defined before this script loads.
 *
 * Originally authored by LiamStanziani in openo-beta/open-o PR #2261.
 * Ported to CARLOS EMR.
 *
 * @since 2026-01-30
 */
(function() {
    ...
    // Intercept the current CARLOS AJAX utility.
    if (typeof CarlosAjax !== 'undefined') {
        if (typeof CarlosAjax.request === 'function') {
            var originalCarlosRequest = CarlosAjax.request;
            CarlosAjax.request = function(url, options) {
                options = options || {};
                ...
                return originalCarlosRequest.call(CarlosAjax, url, options);
            };
        }

        if (typeof CarlosAjax.updater === 'function') {
            var originalCarlosUpdater = CarlosAjax.updater;
            CarlosAjax.updater = function(container, url, options) {
                options = options || {};
                if (!hasDemographicNo(options.parameters)) {
                    options.parameters = addDemographicNo(options.parameters);
                }
                ...
                return originalCarlosUpdater.call(CarlosAjax, container, url, options);
            };
        }
    }
