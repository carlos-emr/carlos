...
//                                                        strsearchappointmentday = "searchappointmentdaywithlocation";
                                                ...
                                                <tr>
                                                    <td class="<%=bColorHour?"scheduleTime00":"scheduleTimeNot00"%>">
                                                        <a href="javascript:void(0)"
                                                           onClick="confirmPopupPage(600,780, '<%= request.getContextPath() %>/appointment/addappointment.jsp?provider_no=<%=Encode.forJavaScript(curProvider_no[nProvider])%>&bFirstDisp=<%=true%>&year=<%=Encode.forJavaScript(strYear)%>&month=<%=Encode.forJavaScript(strMonth)%>&day=<%=Encode.forJavaScript(strDay)%>&start_time=<%=Encode.forJavaScript((hourCursor>9?(""+hourCursor):("0"+hourCursor))+":"+ (minuteCursor<10?"0":"") +minuteCursor)%>&end_time=<%=Encode.forJavaScript((hourCursor>9?(""+hourCursor):("0"+hourCursor))+":"+(minuteCursor+depth-1))%>&duration=<%=Encode.forJavaScript(dateTimeCodeBean.get("duration"+hourmin.toString()) != null ? (String) dateTimeCodeBean.get("duration"+hourmin.toString()) : "")%>','<%=Encode.forJavaScript(dateTimeCodeBean.get("confirm"+hourmin.toString()) != null ? (String) dateTimeCodeBean.get("confirm"+hourmin.toString()) : "")%>','<%=Encode.forJavaScript(allowDay)%>','<%=Encode.forJavaScript(allowWeek)%>');return false;"
