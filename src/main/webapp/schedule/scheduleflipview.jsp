    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function changePro(providerno) {
                a = "scheduleflipview.jsp?originalpage=<%=Encode.forJavaScript(originalPage)%>&provider_no=" + providerno +<%=request.getParameter("startDate")!=null?("\"&startDate="+request.getParameter("startDate")+"\""):"\""%>;
                self.location.href = a;
            }

            function selectprovider(s) {
                a = "scheduleflipview.jsp?originalpage=<%=Encode.forJavaScript(originalPage)%>&provider_no=" + s.options[s.selectedIndex].value +<%=request.getParameter("startDate")!=null?("\"&startDate="+request.getParameter("startDate")+"\""):"\""%>;
                self.location.href = a;
            }//-->
...
//  String dateString1 = outform.format(inform.parse(cal.get(Calendar.YEAR)+"-"+(cal.get(Calendar.MONTH)+1)+"-"+cal.get(Calendar.DATE)) );
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0">
        <tr align="center" bgcolor="#CCCCFF">
            ...
            <td width="15%" nowrap><a
                    href="scheduleflipview.jsp?originalpage=<%=Encode.forHtmlAttribute(originalPage)%>&provider_no=<%=Encode.forHtmlAttribute(curProvider_no)%>&startDate=<%=lastMonth.get(Calendar.YEAR)+"-"+(lastMonth.get(Calendar.MONTH)+1)+"-"+lastMonth.get(Calendar.DATE)%>"
                ...
                %>
                    ...
                    href="scheduleflipview.jsp?originalpage=<%=Encode.forHtmlAttribute(originalPage)%>&provider_no=<%=Encode.forHtmlAttribute(curProvider_no)%>&startDate=<%=nextMonth.get(Calendar.YEAR)+"-"+(nextMonth.get(Calendar.MONTH)+1)+"-"+nextMonth.get(Calendar.DATE)%>"
    ...
    <a
            href="scheduleflipview.jsp?originalpage=<%=Encode.forHtmlAttribute(originalPage)%>&provider_no=<%=Encode.forHtmlAttribute(curProvider_no)%>&startDate=<%=lastMonth.get(Calendar.YEAR)+"-"+(lastMonth.get(Calendar.MONTH)+1)+"-"+lastMonth.get(Calendar.DATE)%>"><fmt:setBundle basename="oscarResources"/><fmt:message key="schedule.scheduleflipview.btnLastMonth"/> </a>
    |
    <a
            href="scheduleflipview.jsp?originalpage=<%=Encode.forHtmlAttribute(originalPage)%>&provider_no=<%=Encode.forHtmlAttribute(curProvider_no)%>&startDate=<%=nextMonth.get(Calendar.YEAR)+"-"+(nextMonth.get(Calendar.MONTH)+1)+"-"+nextMonth.get(Calendar.DATE)%>"><fmt:setBundle basename="oscarResources"/><fmt:message key="schedule.scheduleflipview.btnNextMonth"/></a>
    </body>
</html>
