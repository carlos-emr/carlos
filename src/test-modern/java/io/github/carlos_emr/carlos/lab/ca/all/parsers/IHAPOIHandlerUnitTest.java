 ...
 *
 * <p>
 * Migrated from legacy JUnit 4 IHAPOIHandlerTest to JUnit 5 for the CARLOS EMR project (2026).
 */
...
/**
 * Unit tests for {@link IHAPOIHandler} (Interior Health Authority BC HL7 parser).
 *
 * <p>Parameterized tests that validate HL7 message parsing across all test data
 * from the IHAPOI test archive. Each HL7 message from the ZIP is parsed and
 * validated for correct extraction of demographics, results, and metadata.
 * Migrated from legacy JUnit 4 parameterized IHAPOIHandlerTest.
 *
...
@DisplayName("IHAPOIHandler parameterized unit tests")
class IHAPOIHandlerUnitTest {
    ...
    private IHAPOIHandler createHandler(String hl7Body) throws HL7Exception {
    ...
    @DisplayName("should return MEDITECH as message type")
    void shouldReturnMsgType_asMEDITECH(String hl7Body) throws HL7Exception {
        IHAPOIHandler handler = createHandler(hl7Body);
        assertThat(handler.getMsgType()).isEqualTo("MEDITECH");
    }
    ...
    @ParameterizedTest
    @MethodSource("hl7MessageProvider")
    @DisplayName("should return empty message priority")
    void shouldReturnEmptyMsgPriority(String hl7Body) throws HL7Exception {
        IHAPOIHandler handler = createHandler(hl7Body);
        assertThat(handler.getMsgPriority()).isEmpty();
    }
    ...
    @ParameterizedTest
    @MethodSource("hl7MessageProvider")
    @DisplayName("should parse OBX value types for all segments")
    void shouldParseOBXValueTypes_forAllSegments(String hl7Body) throws HL7Exception {
        IHAPOIHandler handler = createHandler(hl7Body);
        for (int i = 0; i < handler.getOBRCount(); i++) {
            for (int j = 0; j < handler.getOBXCount(i); j++) {
                assertThat(handler.getOBXValueType(i, j)).isNotNull();
            }
        }
    }
    ...
    @ParameterizedTest
    @MethodSource("hl7MessageProvider")
    @DisplayName("should parse timestamp for first OBX segment")
    void shouldParseTimestamp_forFirstOBXSegment(String hl7Body) throws HL7Exception {
        IHAPOIHandler handler = createHandler(hl7Body);
        assertThat(handler.getTimeStamp(0, 0)).isNotNull();
    }
    ...
    @ParameterizedTest
    @MethodSource("hl7MessageProvider")
    @DisplayName("should determine structured vs unstructured status")
    void shouldDetermineStructuredStatus(String hl7Body) throws HL7Exception {
        IHAPOIHandler handler = createHandler(hl7Body);
        boolean isUnstructured = handler.isUnstructured();
        assertThat(isUnstructured).isEqualTo(
                !IHAPOIHandler.STRUCTURED.LAB.name().equalsIgnoreCase(handler.getDiagnosticServiceId()));
    }
}
