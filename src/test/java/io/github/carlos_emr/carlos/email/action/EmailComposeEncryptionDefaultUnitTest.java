...
class EmailComposeEncryptionDefaultUnitTest extends CarlosUnitTestBase {
    ...
    private MockHttpServletRequest prepareComposer(
        ...
        EmailComposeSubmissionStateService composeSubmissionStateService =
                new EmailComposeSubmissionStateService();
        ...
        when(emailPdfPasswordService.generatePassphrase()).thenReturn("example-generated-passphrase");
        ...
        when(emailComposeManager.prepareEFormAttachments(
                any(), any(), any(), any(EmailComposeWorkingDirectory.class))).thenReturn(List.of());
        when(emailComposeManager.prepareEDocAttachments(
                any(), any(), any(EmailComposeWorkingDirectory.class))).thenReturn(List.of());
        when(emailComposeManager.prepareLabAttachments(
                any(), any(), any(EmailComposeWorkingDirectory.class))).thenReturn(List.of());
        when(emailComposeManager.prepareHRMAttachments(
                any(), any(), any(EmailComposeWorkingDirectory.class))).thenReturn(List.of());
        when(emailComposeManager.prepareFormAttachments(
                any(), any(), any(), anyInt(), any(EmailComposeWorkingDirectory.class))).thenReturn(List.of());
