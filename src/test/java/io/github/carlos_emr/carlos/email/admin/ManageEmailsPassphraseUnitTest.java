...
class ManageEmailsPassphraseUnitTest extends CarlosUnitTestBase {
    ...
    void shouldClearStaleComposeState_whenWorkingDirectoryCreationFails() {
        ...
        LoggedInInfo.setLoggedInInfoIntoSession(request.getSession(), new LoggedInInfo());
        EmailLog patientLog = new EmailLog();
        Demographic patient = new Demographic();
        patient.setDemographicNo(123);
        patientLog.setDemographic(patient);
        when(emailComposeManager.prepareEmailForResend(any(), anyInt())).thenReturn(patientLog);
