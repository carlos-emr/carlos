...
@Tag("unit")
@Tag("service")
class SmsRetryCalculatorUnitTest {
    ...
    void shouldAllowRetry_whenAttemptsRemain() {
        SmsRetryCalculator retryPolicy = new SmsRetryCalculator(3, Duration.ofMinutes(1), Duration.ofHours(1));
    ...
    void shouldRejectRetry_whenMaximumAttemptsReached() {
        SmsRetryCalculator retryPolicy = new SmsRetryCalculator(2, Duration.ofMinutes(1), Duration.ofHours(1));
        SmsTransaction transaction = queuedTransaction();
