...
class BuildInfoUnitTest {
    ...
    void shouldLoadStamp_fromClasspath() {
        // The project version is written into target/classes/carlos-build.properties by Maven
        // resource filtering, which pom.xml enables unconditionally for src/main/resources, and the
        // tests only run under that build. So the version MUST be present here: an "unknown" tag
        // is exactly the "build version never updates" regression this PR fixes, and accepting it
        // (as an earlier version of this test did for unfiltered IDE builds) let a renamed key or
        // disabled filtering pass silently.
        ...
        assertThat(info.getBuildTag()).isNotEqualTo("unknown").doesNotContain("${").startsWith(info.getVersion());
    }
