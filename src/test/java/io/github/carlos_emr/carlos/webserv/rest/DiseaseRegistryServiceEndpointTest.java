...
/**
 * CXF local-transport endpoint tests for {@link DiseaseRegistryService} using CXF local transport.
 *
...
class DiseaseRegistryServiceEndpointTest extends CarlosRestTestBase {
    ...
    void shouldRejectAccessBeforeLookup_whenPrivilegeIsDenied() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> request().path("/dxRegisty/quickLists").get())
        ...
        org.mockito.Mockito.verifyNoInteractions(mockQuickListDao);
    }
    ...
    class GetDiseaseRegistry {
        ...
        void shouldReturn200WithEntries_whenEntriesExist() {
            ...
            assertThat(wireJson.at("/0/demographicNo").intValue()).isEqualTo(123);
            assertThat(wireJson.at("/0/status").textValue()).isEqualTo("A");
            assertThat(wireJson.at("/0/dxresearchCode").textValue()).isEqualTo("250");
            assertThat(wireJson.at("/0/codingSystem").textValue()).isEqualTo("ICD9");
        }
