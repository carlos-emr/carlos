...
/**
 * CXF local-transport endpoint tests for {@link DocumentService} using CXF local transport.
 *
...
class DocumentServiceEndpointTest extends CarlosRestTestBase {
    ...
    class SaveDocumentToDemographic {
        ...
        void shouldReturn200_whenDocumentSavedSuccessfully() throws IOException {
            ...
            assertThat(wireJson.at("/reviews").isArray()).isTrue();
            assertThat(wireJson.at("/reviews")).hasSize(0);
        }
        ...
        void shouldReturn400_whenRequiredFieldsMissing() {
            ...
            assertThat(response.readEntity(String.class)).isEqualTo(
                    "The request body must contain a title, encoded documentData, a fileType (png, jpg, pdf, etc.), and a demographicNo");
            org.mockito.Mockito.verifyNoInteractions(mockDocumentManager);
        }
        ...
        void shouldReturn500_whenDocumentSaveThrowsIOException() throws IOException {
            ...
            assertThat(response.readEntity(String.class)).isEqualTo("The document could not be saved.");
        }
    }
    ...
    class UploadPendingDocuments {
        ...
        void shouldReturn400_whenRequiredFieldsMissing() {
            ...
            assertThat(response.getStatus()).isEqualTo(400);
            var wireJson = responseJson(response);
            assertThat(wireJson.at("/message").textValue()).isEqualTo("Missing required fields: fileName, fileContent, contentType, or queue");
            assertThat(wireJson.at("/status").textValue()).isEqualTo("Failed");
            org.mockito.Mockito.verifyNoInteractions(mockDocumentManager);
        }
        ...
        void shouldReturn403_whenAccessDenied() {
            ...
            var wireJson = responseJson(response);
            assertThat(wireJson.at("/message").textValue()).isEqualTo("Access Denied");
            assertThat(wireJson.at("/status").textValue()).isEqualTo("Failed");
        }
    }
}
