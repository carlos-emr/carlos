...
/**
 * CXF local-transport endpoint tests for {@link EFormsService} using CXF local transport.
 *
 * @since 2026-03-31
...
class EFormsServiceEndpointTest extends CarlosRestTestBase {
    ...
    class GetEFormList {
        ...
        void shouldReturn200_whenEFormsExist() {
            ...
            assertThat(wireJson.at("/body").isArray()).isTrue();
            assertThat(wireJson.at("/body")).hasSize(1);
            assertThat(wireJson.at("/body/0/formName").textValue()).isEqualTo("Test Form");
        ...
        void shouldReturn200WithEmptyList_whenNoEFormsExist() {
            ...
            var wireJson = responseJson(response);
            assertThat(wireJson.at("/body").isArray()).isTrue();
            assertThat(wireJson.at("/body")).hasSize(0);
            assertThat(wireJson.at("/status").textValue()).isEqualTo("SUCCESS");
        }
    }
}
