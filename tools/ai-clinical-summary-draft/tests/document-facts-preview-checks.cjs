...
(async () => {
    ...
    try {
        ...
        const example = page.locator('.case').filter({ has: page.locator('.candidate table') }).first();
        ...
        await example.locator('.candidate table details summary').first().click();
        assert(await example.locator('.candidate table blockquote').first().isVisible());
